...
const ProfileList = () => {
    ...
    const table_columns = [
        ...
        {
            name: "Name",
            selector: (row) => <span onClick={() => navigate('/add-permission', { state: { permissionViewId: row } })} className='cursor-pointer'>{row?.Role}</span>,
            sortable: true,
            width: "10rem",
        },
        {
...

        {
            ...
            selector: (row) => <div>{row?.DashboardPermissions?.length > 0 &&
                row?.DashboardPermissions.map((data, index) => (
                    <span key={index}>
                        {data?.DashboardPermissionName}
                        {index < row?.DashboardPermissions?.length - 1 && ", "}
                    </span>
                ))}</div>,
            sortable: true,
            // width: "10rem",
    ...
    const handleEdit = (value) => {
        navigate('/add-permission', { state: { permissionViewId: value } })
