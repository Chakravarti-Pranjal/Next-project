...
import PrintInvoiceComponent from "../../Invoices/PrintInvoiceComponent";
...
const AgentTotalInvoice = ({ supplierData, totalAmount, handleInvoiceCloses }) => {
  ...
  console.log(companylogo, "setcompanylogo")

  useEffect(() => {
    if (isEditMode && state?.row) {
      const invoiceDetails = state.row.InvoiceDetails;
      console.log(invoiceDetails, "GFHD&66");
  ...
  useEffect(() => {
    ...
    if (sendInvoiceType === "FileWise" && fileWisePaymentList?.length > 0) {
      ...
      setFormValue((prev) => ({
        ...
        invoiceDetails: [
          {
            ...initialFormValue.invoiceDetails[0], // copy first row from initial form
            Amount: totalAmount,        // overwrite Amount
          },
        ],
      }));
    }
  }, [sendInvoiceType, fileWisePaymentList, itemWisePaymentList, isEditMode, totalAmount]);
  ...
  //     setFormValue((prev) => ({
  //       ...prev,
  //       invoiceDetails: prev.invoiceDetails.map((item) => ({
  //         ...item,
  ...
  //   console.log(formValue, "ARADSE5545")
  // }, [totalAmount]);
...
export default AgentTotalInvoice;
