...
const InvoiveList = () => {
  ...
  const handleNext = () => {
    navigate("/invoices/invoice-show", { state: sendInvoiceType });
  };

  const handleVisit = (row) => {
    console.log("BankDetails in row:", row?.InvoiceDetails?.BankDetails);
    navigate(`/invoices/invoice-show`, {
      state: { row },
    });
  };
  ...
  //     const matchQueryId = invoiceQueryId
  //       ? item?.QueryId?.toString().includes(invoiceQueryId)
  //       : true;

  //     return matchType && matchId && matchQueryId;
  //   });
      ...
      <div className="d-flex mb-2 align-items-end">
        ...
        <div className="col-2 me-2">
          ...
          <input
            ...
            onChange={(e) => setInvoiceQueryId(e.target.value)}
          />
        </div>
        <button
        ...
        {/* <div >
          <button className="btn btn-dark btn-custom-size me-2 my-2" onClick={() => navigate(-1)}>
