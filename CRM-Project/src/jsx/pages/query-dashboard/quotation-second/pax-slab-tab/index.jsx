...
const PaxSlab = ({ paxSlab }) => {
  ...
  const [isExtraBedAvailable, setIsExtraBedAvailable] = useState(false);
  ...
  const getDefaultFormValue = (pax, index, queryData, qoutationData, costs) => {
    ...
    const calculatedCost =
      dividingFactor > 0
        ...
        : 0;
    console.log(costs, "WRWRW8777", pax);

    // ----------------------- Hotel Cost Logic Start

    const hotelFinalCost =
      pax?.Max == 1
        ? Number(costs?.singleHotelCost || 0)
        : Number(costs?.hotelCost || 0) / 2;

    const doublePlusExtrabedCost =
      Number(costs?.hotelCost || 0) + Number(costs?.extrabedHotelCost || 0);

    const doublePlusExtrabedCostFinalCost =
      pax?.Max == 3 && isExtraBedAvailable
        ? Number(doublePlusExtrabedCost / 3)
        : hotelFinalCost;

    // ----------------------- Hotel Cost Logic End
    ...
    return {
      ...
      Hotel: Math.ceil(doublePlusExtrabedCostFinalCost || 0),
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          quotationResponse.data?.data[0]?.QueryInfo?.Accomondation?.RoomInfo
            ?.length > 0
        ) {
          const findExtraBed =
            quotationResponse.data?.data[0]?.QueryInfo?.Accomondation?.RoomInfo?.find(
              (list) => list?.id == 7
            );
          const isUserTakeExtraBed = findExtraBed?.NoOfPax ? true : false;
          setIsExtraBedAvailable(isUserTakeExtraBed);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (listQueryQuotationData.length) {
      const calculateServiceCosts = () => {
        ...
        console.log(listQueryQuotationData, "listQueryQuotationData4747");
        ...
        const doubleRoomCost = hotel?.TotalCosting?.[0]?.HotelRoomBedType?.find(
          (list) => list.RoomBedType == 4
        );
        const singleRoomCost = hotel?.TotalCosting?.[0]?.HotelRoomBedType?.find(
          (list) => list.RoomBedType == 3
        );

        const extraBedCost = hotel?.TotalCosting?.[0]?.HotelRoomBedType?.find(
          (list) => list.RoomBedType == 7
        );

        setCosts((prev) => ({
          ...prev,
          hotelCost: Math.ceil(doubleRoomCost?.ServiceCost || 0),
          singleHotelCost: Math.ceil(singleRoomCost?.ServiceCost || 0),
          extrabedHotelCost: Math.ceil(extraBedCost?.ServiceCost || 0),
          transport:
          ...
          if (
            ...
            const calculatedCost =
              newDivFact > 0
                ...
                : 0;
            console.log(costs, "SGAFT76", newMaxPax);
            ...
            const doublePlusExtrabedCost =
              ...
              Number(costs?.extrabedHotelCost || 0);

            const doublePlusExtrabedCostFinalCost =
              newMaxPax == 3 && isExtraBedAvailable
                ? Number(doublePlusExtrabedCost / 3)
                : hotelCost;
