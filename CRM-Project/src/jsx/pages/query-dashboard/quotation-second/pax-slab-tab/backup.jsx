...
const PaxSlab = ({ paxSlab }) => {
  ...
  const calculateNett = (slab, calculatedTransportCost, additionalSlabCost) => {
    ...
    console.log(calculatedTransportCost, "VCGDGDGGDGDGDG", additionalSlabCost);
    return Math.ceil(
      [
            ...
            updatedItem.DividingFactor > 0
              ? Math.ceil(
                  (transportCost * (parseInt(updatedItem.NoOfVehicle) || 0) +
                    supplimentTransportCost *
                      (parseInt(updatedItem.SupplimentNoOfVehicle) || 0) +
                    (index == 0 ? assistance : 0)) / // Add assistance only to the first row, or adjust as needed
  ...
  const handleIncrementTable = (index) => {
    const newItem = {
      ...
      Transport:
        Math.ceil(
          (costs.transport.find(
            (data) => data.id == slabSingleFormValue[index].TransportType
          )?.TotalServiceCost || 0) *
            (parseInt(slabSingleFormValue[index].NoOfVehicle) || 0) +
            (costs.transport.find(
              (data) =>
                data.id == slabSingleFormValue[index].SupplimentTransferType
            )?.TotalServiceCost || 0) *
              (parseInt(slabSingleFormValue[index].SupplimentNoOfVehicle) ||
                0) +
            assistance // always add assistance
        ) /
        (slabSingleFormValue[index].DividingFactor > 0
          ? slabSingleFormValue[index].DividingFactor
          : 1),
    };
    setSlabSingleFormValue((prev) => [
  ...
  const handleSubmit = async () => {
    const singleSlabPayload = slabSingleFormValue.map((row, idx) => {
      ...
      const totalGst = (nettValue + row.TopMargin) / (gstInfo?.GstValue / 100);
      const totalValue = Math.ceil(
        nettValue + (parseFloat(row.TopMargin) || 0) + totalGst + row.TopMargin
      );

      // const totalUSD = totalValue / ConversionValue
      ...
      return {
        ...
        TotalInUSD: Math.ceil(totalValue / row?.ConversionValue),
      };
    });
      ...
      <tr key={index}>
        ...
        <td>
          <input
            ...
            value={Math.ceil(
              ((costs.transport.find((data) => data.id == slab.TransportType)
                ?.TotalServiceCost || 0) *
                (parseInt(slab.NoOfVehicle) || 0) +
                (costs.transport.find(
                  (data) => data.id == slab.SupplimentTransferType
                )?.TotalServiceCost || 0) *
                  (parseInt(slab.SupplimentNoOfVehicle) || 0) +
                assistance) / // always add assistance, not just for index 0
                (slab.DividingFactor > 0 ? slab.DividingFactor : 1)
            )}
            onChange={(e) => handleFormChange(e, index)}
          />
        </td>
        ...
        <td>
          <input
            ...
            className="formControl1 width50px"
            name="Train"
            value={Math.ceil(slab.Train ? slab.Train : "") || ""}
            onChange={(e) => {
              const input = e.target.value;
              handleFormChange(
                {
        ...
        <td>
          <input
            ...
            className="formControl1 width50px"
            name="LocalEscort"
            value={Math.ceil(slab.LocalEscort?.[0]?.Total || 0)}
            onChange={(e) => handleFormChange(e, index)}
          />
        </td>
        <td>
        ...
        <table className="table table-bordered itinerary-table table-first-child">
          <thead>
            <tr>
              ...
              <th>Total</th>
              <th>Total in {currencyType}</th>
              <th>Air in {currencyType}</th>
            </tr>
          </thead>
          <tbody>
