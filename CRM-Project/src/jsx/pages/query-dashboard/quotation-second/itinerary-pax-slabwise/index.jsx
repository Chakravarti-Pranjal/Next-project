...
const PaxSlab = ({ Type }) => {
  ...
  const {
    toggleLocalForeigner,
    incrementLocalForeignerSlab,
    incrementMainSlab,
  } = useSelector((state) => state?.createExcortDayLocalForeignerReducer);
  console.log(incrementMainSlab, "WTSDSTT655");
  const initialized = useRef(false);

  useEffect(() => {
    console.log(QoutationData?.ExcortDays, "ETDFFDFDFDFDFD");
    ...
    console.log(foreignerSlabData, "BCHHDGD*77");

    if (paxslabList?.length > 0 && slabFormValue?.length === 0) {
      console.log(slabFormValue, "DFSGST", paxslabList);
      if (foreignerSlabData?.ExcortSlabCost?.length > 0) {
        ...
        // dispatch(toggleMainPaxSlabForignerSlab(1));
      }
    }

    // if (!initialized.current && foreignerSlabData?.ExcortSlabCost?.length > 0) {
    //   setSlabFormValue(foreignerSlabData.ExcortSlabCost);
    // }
  }, [paxslabList, QoutationData]);

  console.log(slabFormValue, "WRSFFD666");
  ...
  const fetchQueryQuotationData = async () => {
    try {
      ...
      console.log(data?.data[0], "WQRFD87777");
    } catch (error) {
  ...
  useEffect(() => {
    if (QoutationData?.ExcortDays?.length > 0) {
      ...
      const monument = foreignerQuotationData?.Days?.[0]?.DayServices?.filter(
      ...
      const monumentServices = foreignerQuotationData?.Days?.flatMap((day) =>
        day.DayServices.filter((service) => service.ServiceType === "Monument")
      );
      const additionalService = foreignerQuotationData?.Days?.flatMap((day) =>
        day.DayServices.filter(
      ...
      if (additionalService?.length > 0) {
        additionalCost = additionalService?.reduce((sum, item) => {
          const itemSum = item.AdditionalCost.reduce((innerSum, cost) => {
      ...
      console.log(additionalCost, "additionalService");

      let mounemntCost = 0;

      if (monumentServices?.length > 0) {
        mounemntCost = monumentServices?.reduce((sum, item) => {
          const fAdultCost = item.ServiceDetails[0].ItemUnitCost.FAdultCost;
          return sum + (parseFloat(fAdultCost) || 0);
        }, 0);
      }
    ...
    if (slabFormValue?.length > 0) {
      const updatedList = slabFormValue.map((slab) => {
        ...
        return {
          ...
          Air: serviceCost.Air,
          Additional: serviceCost?.Additional,
        };
      });
  ...
  // Store base values for each row and field
  ...
  const handleInputChange = async (index, field, value) => {
    setSlabFormValue((prev) => {
      ...
      // FOC logic: always multiply the original (base) value
      const foc = parseInt(updatedList[index].Foc) || 0;
      const fieldsToMultiply = [
        "Fee",
        "Meal",
        "Activity",
        "Additional",
      ...
      // Initialize base values for this row if not present
      setBaseValues((prevBase) => {
        const newBase = { ...prevBase };
        if (!newBase[index]) newBase[index] = {};
        fieldsToMultiply.forEach((f) => {
          // If base value not set or field is being edited, update base value
          if (typeof newBase[index][f] === "undefined" || field === f) {
            newBase[index][f] =
              field === f
                ? parseFloat(value) || 0
                : parseFloat(updatedList[index][f]) || 0;
          }
        });
        return newBase;
      });

      // Use a closure to get the latest base values after setBaseValues
      setTimeout(() => {
        setSlabFormValue((prev2) => {
          const updatedList2 = [...prev2];
          const base = baseValues[index] || {};
          fieldsToMultiply.forEach((f) => {
            const baseVal =
              base && typeof base[f] !== "undefined"
                ? base[f]
                : parseFloat(updatedList2[index][f]) || 0;
            updatedList2[index][f] = foc > 0 ? baseVal * foc : baseVal;
          });
          // Calculate Hotel cost based on DBL and SGL
          let hotelCost = 0;
          const doubleValue = parseFloat(updatedList2[index].Double) || 0;
          const singleValue = parseFloat(updatedList2[index].Single) || 0;
          const doubleType = updatedList2[index].DoubleSelectType;
          const singleType = updatedList2[index].SingleSelectType;
          ...
          if (field === "Double" || field === "Single") {
            ...
            updatedList2[index].Hotel = Math.ceil(hotelCost);
          }
          updatedList2[index].TotalPerPerson = Math.ceil(
            (parseFloat(updatedList2[index].Hotel) || 0) +
              (parseFloat(updatedList2[index].Fee) || 0) +
              (parseFloat(updatedList2[index].Meal) || 0) +
              (parseFloat(updatedList2[index].Activity) || 0) +
              (parseFloat(updatedList2[index].Additional) || 0) +
              (parseFloat(updatedList2[index].Train) || 0) +
              (parseFloat(updatedList2[index].Air) || 0)
          );
          return updatedList2;
        });
      }, 0);
  ...
  return (
    <div className="row mt-3 m-0">
      <div className="col-12 px-1 py-2 d-flex justify-content-between itinerary-head-bg foreignerEscort-head-bg">
