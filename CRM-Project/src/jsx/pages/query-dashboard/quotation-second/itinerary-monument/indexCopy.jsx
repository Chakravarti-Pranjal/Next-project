  ...
  const filterMonumentPackageList = (packageId, index) => {
    ...
    setMultipleMonument((prevList) => {
      ...
      if (monumentFromValue[index]?.ServiceId === packageId) {
        // Optional: filter those that actually have RateJson
        const monumentsWithRates = normalizedMonuments.filter((monument) =>
          Array.isArray(monument.RateJson)
        );
...
const handleIndianFeeChange = (e, monumentId) => {
  ...
  const newValue = isNaN(parsed) ? "" : parsed; // safe NaN handling
  ...
  setChangeMonument((prev) => {
    const updatedMonuments = (prev.monument || []).map((m) => {
      if (m.id === monumentId) {
        ...
        updatedRateJson.Data[0].RateDetails =
          updatedRateJson.Data[0].RateDetails || [{}];

<<<<<<< HEAD
          // ✅ Case 1: Agar RateJson ek array hai
          if (Array.isArray(updatedRateJson)) {
            updatedRateJson[0] = {
              ...updatedRateJson[0],
              IndianAdultEntFee: newValue.toString(),
            };
          }
          // ✅ Case 2: Agar RateJson nested object hai (Data -> RateDetails)
          else if (updatedRateJson?.Data) {
            updatedRateJson.Data = updatedRateJson.Data || [{}];
            updatedRateJson.Data[0] = updatedRateJson.Data[0] || {};
...
=======
        // ✅ Preserve existing fields while updating IndianAdultEntFee
        updatedRateJson.Data[0].RateDetails[0] = {
          ...updatedRateJson.Data[0].RateDetails[0],
          IndianAdultEntFee: newValue.toString(),
        };

        // Case 1: Array type RateJson
        if (Array.isArray(updatedRateJson)) {
          updatedRateJson[0] = {
            ...updatedRateJson[0],
            IndianAdultEntFee: newValue.toString(),
        ...
        // Case 2: Nested object already handled above ✅
...
>>>>>>> 47b6066925d871513f272aef0de1e9c3e408b65e
...
<<<<<<< HEAD
  const handleForeignFeeChange = (e, monumentId) => {
    ...
    console.log("Foreign Fee newValue:", newValue);
=======
...
=======
        // ✅ Preserve existing fields while updating ForeignerAdultEntFee
        updatedRateJson.Data[0].RateDetails[0] = {
          ...updatedRateJson.Data[0].RateDetails[0],
          ForeignerAdultEntFee: newValue.toString(),
        };
        ...
        // Case 3: Simple object
          ...
          console.log(updatedRateJson, 'updateRateJson')
>>>>>>> 47b6066925d871513f272aef0de1e9c3e408b65e
                            ...
                            {monumentMasterList?.map((monument) => {
                              ...
                              const handleCheckboxChange = () => {
                                if (isChecked) {
                                  ...
                                  // console.log(deepCopiedRateJson,"deepCopiedRateJson");
...
<<<<<<< HEAD
                              ...
                              {/* <td>
                                {changeMonument?.monument.reduce(
                                  (total, curr) => {
                                    const fee =
                                      curr?.RateJson?.[0]?.IndianAdultEntFee || // case: array
                                      curr?.RateJson?.IndianAdultEntFee || // case: simple object
                                      curr?.RateJson?.Data?.[0]
                                        ?.RateDetails?.[0]?.IndianAdultEntFee; // case: nested object
                              ...
                              <td>
                                {changeMonument?.monument.reduce(
                                  (total, curr) => {
                                    const fee =
                                      // parseFloat(curr?.RateJson?.ForeignerAdultEntFee) || // Primary case: top-level fee
                                      parseFloat(
                                    ...
                                    0; // Default to 0 if no valid fee is found
...
=======
                              ...
                              <td>
                                ...
                                const fee =
                                  Array.isArray(curr?.RateJson) && curr?.RateJson[0]?.IndianAdultEntFee
                                    ? curr.RateJson[0].IndianAdultEntFee
                                    : curr?.RateJson?.IndianAdultEntFee &&
                                      parseFloat(curr.RateJson.IndianAdultEntFee) > 0
                                    ? curr.RateJson.IndianAdultEntFee
                                    : curr?.RateJson?.Data?.[0]?.RateDetails?.[0]?.IndianAdultEntFee;
...
=======
                                ...
                                const fee =
                                  ...
                                  curr?.RateJson?.Data?.[0]?.RateDetails?.[0]?.ForeignerAdultEntFee
                                    ? curr.RateJson.Data[0].RateDetails[0].ForeignerAdultEntFee
                                    // Priority 2: Array style RateJson
                                    : Array.isArray(curr?.RateJson) && curr?.RateJson[0]?.ForeignerAdultEntFee
