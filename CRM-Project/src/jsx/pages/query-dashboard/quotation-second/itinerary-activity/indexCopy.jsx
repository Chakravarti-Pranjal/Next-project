...
const Activity = ({ Type, checkBoxes, headerDropdown }) => {
      ...
      if (hasMonumentService) {
        ...
        qoutationData?.Days?.forEach((day, dayIndex) => {
          const services =
            day?.DayServices?.filter(
              (s) =>
                s?.ServiceType === "Activity" && s?.ServiceMainType === "Guest"
            ) || [];

          console.log(services, "services410");

          services.forEach((service, serviceIndex) => {
            // console.log("ServiceAdditionalCost87", service?.AdditionalCost);
            // console.log("Type:01", typeof service?.AdditionalCost, Array.isArray(service?.AdditionalCost));
            // console.log("Raw:02", service?.AdditionalCost);
            ...
            // Store by dayIndex and serviceIndex
            // initialRows[`${dayIndex}_${serviceIndex}`] = mappedRows;
            initialRows[`${dayIndex}`] = mappedRows;
          });
        });

        console.log(initialRows, "initialRows145");
  ...
  }, [qoutationData, checkBoxes, activityList, supplierList]);
