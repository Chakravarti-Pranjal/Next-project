...
const Hotel = ({ headerDropdown, Type, ActiveOptionId, setHeaderDropdown }) => {
  ...
  const [HotelCategorychanges, setHotelCategorychanges] = useState(false);
  ...
  // State for automatic copy-paste
  const [copiedRates, setCopiedRates] = useState({
    dbl: null,
    sgl: null,
    extraBed: null,
    twin: null,
  ...
  // Function to copy rates and automatically paste to NEXT row
  const handleCopyRates = (index) => {
    // Ensure roomDetails and mealDetails exist for the current index
    if (!roomDetails[index] || !mealDetails[index]) {
      console.warn("roomDetails or mealDetails not found at index:", index);
      // notifyError("No data to copy at this row!");
      return;
    }

    const rooms = roomDetails[index];
    const meals = mealDetails[index];

    // Extract room rates
    const dbl = rooms.find((r) => r.RoomBedTypeId === 4)?.RoomCost || 0;
    const sgl = rooms.find((r) => r.RoomBedTypeId === 3)?.RoomCost || 0;
    const extraBed = rooms.find((r) => r.RoomBedTypeId === 7)?.RoomCost || 0;
    const twin = rooms.find((r) => r.RoomBedTypeId === 5)?.RoomCost || 0;
    const tpl = rooms.find((r) => r.RoomBedTypeId === 6)?.RoomCost || 0;
    const ExtraBed = rooms.find((r) => r.RoomBedTypeId === 8)?.RoomCost || 0;

    // Extract meal costs
    const breakfast = meals.find((m) => m.MealTypeId == 1)?.MealCost || 0;
    const lunch = meals.find((m) => m.MealTypeId == 2)?.MealCost || 0;
    const dinner = meals.find((m) => m.MealTypeId == 3)?.MealCost || 0;

    const newCopiedRates = {
      dbl,
      sgl,
      extraBed,
      twin,
    ...
    // Update copiedRates state
    setCopiedRates(newCopiedRates);
    setIsCopied(true);
    setLastCopiedIndex(index);

    // Pass rates to autoPasteToNextRow
    autoPasteToNextRow(index, newCopiedRates);
  };

  // Function to automatically paste to NEXT row
  const autoPasteToNextRow = (currentIndex, ratesToPaste) => {
    const nextIndex = currentIndex + 1;

    // Check if next row exists
    if (
      nextIndex >= roomDetails.length ||
      !roomDetails[nextIndex] ||
      !mealDetails[nextIndex]
    ) {
      // notifyError("Next row not available!");
      console.warn("Next row not available at index:", nextIndex);
      return;
    }
    ...
    setRoomDetails((prev) => {
      ...
      if (newDetails[nextIndex]) {
        newDetails[nextIndex] = newDetails[nextIndex].map((room) => {
          if (room.RoomBedTypeId === 4) return { ...room, RoomCost: dbl };
          if (room.RoomBedTypeId === 3) return { ...room, RoomCost: sgl };
          if (room.RoomBedTypeId === 7) return { ...room, RoomCost: extraBed };
          if (room.RoomBedTypeId === 5) return { ...room, RoomCost: twin };
          if (room.RoomBedTypeId === 6) return { ...room, RoomCost: tpl };
          if (room.RoomBedTypeId === 8) return { ...room, RoomCost: ExtraBed };
          return room;
        });
      }
      return newDetails;
    ...
    setMealDetails((prev) => {
      const newDetails = [...prev];
      if (newDetails[nextIndex]) {
        newDetails[nextIndex] = newDetails[nextIndex].map((meal) => {
          if (meal.MealTypeId == 1)
            return { ...meal, MealCost: breakfast, IsPrice: !!breakfast };
          if (meal.MealTypeId == 2)
            return { ...meal, MealCost: lunch, IsPrice: !!lunch };
          if (meal.MealTypeId == 3)
            return { ...meal, MealCost: dinner, IsPrice: !!dinner };
          return meal;
        });
      }
      return newDetails;
    ...
    // Update originalRoomDetails (if needed)
    ...
    // Update originalMealDetails (if needed)
      ...
      if (hasMonumentService) {
            ...
            initialFormValue.push({
              ...
              Supplier:
                service?.ServiceDetails?.[0]?.ItemSupplierDetail
                  ?.ItemSupplierId || "",
              RateUniqueId: "",
              HotelCategory: service?.HotelCategoryId,
              FromDay: "",
              ToDay: "",
              ItemFromTime: "",
              ItemToTime: "",
        ...
        const staticRoom = initialFormValue?.map(() => {
          ...
          if (isExtraBedAvailable) {
            setHotelCheckBox((prev) => ({
              ...prev,
              original: [...prev.original, "extrabed"],
            }));
        ...
        const roomJson = qoutationData?.Days?.filter(
          ...
          console.log(isExtraBedAvailable, "isExtraBedAvailable");
  ...
  //     setHotelFormValue(updatedForm);
  //   }
  //   console.log("calls how many times");
  // }, [headerDropdown?.Hotel]);
  console.log(hotelFormValue, "hotelFormValue112");
  ...
  const getSupplierList = async (ServiceId, Destination, index) => {
    console.log(ServiceId, Destination, index, "ServiceId, Destination, index");
    ...
    if (hotelName && Destination) {
      console.log("callls how many times ", index);
      ...
      try {
        ...
        console.log(supplier, index, "suppliercheckkkk");
  ...
  console.log(hotelFormValue, "99999999999999");
  ...
  useEffect(() => {
    ...
    if (isServiceIdChangeFromHandle.current) {
      console.log(isServiceIdChangeFromHandle.current, "checkcallls");
  ...
  const getHotelRateApi = async (
    ...
    try {
      ...
      console.log(qoutationData, "quotationData111");
      ...
      console.log(hotelcatogoryvalue, "hotelcatogoryvalue");
      ...
      if (
        ...
        const { data } = await axiosOther.post("priceEditHotelRatesJson", {
          ...
          ValidFrom:
            qoutationData?.TravelDateInfo?.ScheduleType == "Date Wise"
              ? date
              : qoutationData?.TravelDateInfo?.FromDate,
          ValidTo:
            qoutationData?.TravelDateInfo?.ScheduleType == "Date Wise"
              ? date
              : qoutationData?.TravelDateInfo?.ToDate,
          QueryId: "",
          QuatationNo: "",
        });
              ...
              BlackoutDates: [
                {
                  ...
                  BlackoutDatesTo:
                    ...
                    " ",
                  BlackoutDatesRemark:
                    rateData?.RateJson?.BlackoutDates?.[0]
                      ?.BlackoutDatesRemark || " ",
                },
              ],
            };
        ...
        if (data?.Status == 1 && data?.Data?.length > 0) {
          ...
          console.log(hotelFormValue, "hotelFormValue111");

          setHotelFormValue((prevValue) => {
            const newArr = [...prevValue];
            newArr[index] = {
              ...hotelFormValue[index],
              ServiceId: ServiceId || hotelFormValue[index]?.ServiceId,
              RoomCategory:
                rateData?.RateJson?.RoomTypeId || rateData?.RateJson?.RoomTypeID
    ...
    if (hotelFormValue[index]?.id) {
      setHotelFormValue((prevArr) => {
        ...
        newArr[index] = {
          ...
          MealPlan: headerDropdown?.MealPlan,
          HotelCategory: hotelFormValue[index]?.HotelCategory || headerDropdown?.Hotel,
        };
        // console.log(newArr, "11111111111111111111");
          ...
          if (hasHotelService) {
            if (headerDropdown?.HotelData) {
              ...
              setHotelFormValue((prevArr) => {
                ...
                console.log(newArr, "checkthisnewArrr");
              ...
              setHotelFormValue((prevArr) => {
                ...
                console.log(newArr, index, "checknewArrrrr");
  ...
  const handleUpdateRowData = async (hotelRowData) => {
    ...
    const updatedHotelFormValue = hotelFormValue?.map((item) => {
      if (item?.DayNo == dayNo) {
        ...
        return {
          ...
          HotelCategory:
            hotelRowData?.RateDetail?.HotelCategoryId || item?.HotelCategory,
    ...
    // const getHotelList = (index)=>{
    try {
      const limit = "5300";
      const { data } = await axiosOther.post("hotellist", {
        HotelName: "",
        DestinationId: updatedHotelFormValue[dayIndex]?.Destination || "",
        HotelCategoryId: updatedHotelFormValue[dayIndex]?.HotelCategory || " ",
        Default: "Yes",
        perPage: limit,
      });
      ...
      setHotelFullList((prev) => {
        const newList = [...prev];
        newList[dayIndex] = data?.DataList || [];
        return newList;
      });
    ...
    console.log(updatedHotelFormValue, "updatedHotelFormValue");
  ...
  const handleHotelCheckBox = (e, type, index) => {
    ...
    console.log(name, value, "checknamevalue");
  ...
  console.log(hotelCheckBox, "hotelCheckBox");
  ...
  const handleHotelTableDecrement = async (index, item) => {
    if (
      index < 0 ||
    ...
    const confirmation = await swal({
      title: "Do you really want to remove this hotel entry ?",
      // text: "",
      icon: "warning",
      buttons: {
        cancel: {
          ...
          value: false,
          visible: true,
          className: "btn-custom-size btn btn-danger light",
          closeModal: true,
        },
        confirm: {
          text: "Yes, remove it",
  ...
  useEffect(() => {
    isServiceIdChangeFromHandle.current = true;
    setHeaderDropdown({ ...headerDropdown, HotelData: null });
    const { index, field, value } = selectedInd;
    ...
    const fetchHotels = async () => {
      if (field === "HotelCategory") {
        // setHotelCategorychanges(true);

        try {
          const limit = "5300";
          const { data } = await axiosOther.post("hotellist", {
            HotelName: "",
            DestinationId: hotelForm?.Destination,
            HotelCategoryId: value || " ",
            Default: "Yes",
            perPage: limit,
          });
          ...
          console.log(hotelFormValue, "hotelFormValueinn");

          // setHotelCategorychanges(false);

          // ✅ getHotelRateApi call after hotel selection
          if (data?.DataList?.[0]) {
            getHotelRateApi(
              data?.DataList?.[0]?.HotelUniqueID,
              data?.DataList?.[0]?.HotelDestination?.DestinationUniqueId,
              index,
              "",
              hotelForm?.QuatationNo || "",
              hotelForm?.Date || "",
    ...
    console.log(hotelFormValue, "ServiceId1");
    fetchHotels();
  }, [selectedInd]);
  console.log(hotelFormValue, "ServiceId1123");

  // hotel table forms onchange handler
  // Add a ref to track if ServiceId change is from handleHotelFormChange
  console.log(headerDropdown?.hotel, "headerDropdown1111");
  ...
  const seperatefunction = () => {
    ...
    const finalMealDetails = hotelFormValue?.map((_, index) => {
      return mealDetails[index]?.map((meal) => ({
        ...
        MealTypePackage: meal?.IsPrice == true ? "Yes" : "No",
      }));
    });
  ...
  const mealPlan = {
    MealPlanId: headerDropdown?.MealPlan ?? null,
    MealPlanName:
  ...
  return (
    <>
                                ...
                                <span
                                  ...
                                  onClick={() => {
                                    ...
                                    const hotelCategoryId =
                                      hotelFormValue[index]?.HotelCategory;
                                    const RoomCategory =
                                      hotelFormValue[index]?.RoomCategory;
                                    ...
                                    const hotel = getSelectedHotel(index);
                            ...
                            <td>
                              <div>
                                <select
                                  ...
                                  value={hotelFormValue[index]?.HotelCategory}
                                  onChange={(e) =>
                                    handleHotelFormChange(
                                      index,
                                      "",
                                ...
                                <div className="icon-container ">
                                  ...
                                  <p className="tooltip-text py-1 px-1 d-flex flex-column">
                                    {BlackoutDates[index]?.BlackoutDates
                                      ...
                                      <span>No blackout dates available</span>
                                    )}
                                ...
                                {hotelCheckBox?.original?.includes("extrabed")
                                  ? roomDetails[index]?.map((room, ind) => {
                                      return (
                                        <td key={ind + 1}>
                                          <div>
                                            {/* ✅ Copy सिर्फ SGL room पर ही */}
                                            {room?.RoomType === "DBL Room" && (
                                              <>
                                                <span
                                                  onClick={() =>
                                                    handleCopyRates(index)
                                                  }
                                                  className="ms-1 cursor-pointer m-2"
                                                  title="Copy DBL & SGL Rates"
                                                >
      ...
      <HotelDetailModel
        ...
        category={rowDestination?.hotelCategoryId}
        mealPlan={mealPlan}
        paxType={paxType}
