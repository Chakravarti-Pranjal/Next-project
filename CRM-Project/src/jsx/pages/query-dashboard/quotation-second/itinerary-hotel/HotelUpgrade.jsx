...
const HotelUpgrade = ({ headerDropdown, Type, ActiveOptionId, remove }) => {
  ...
  useEffect(() => {
    if (qoutationData?.Days) {
      const hasMonumentService = qoutationData?.Days.some((day) =>
        day?.DayServices?.some(
          (service) =>
            ...
            service.ServiceMainType === "Hotel Suppliment"
        )
      );
      ...
      if (hasMonumentService) {
        ...
        const staticRoom = qoutationData?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        const servicesMeal = qoutationData?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        const staticMeal = qoutationData?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
  ...
  //   const removeHotelAlternate = (index) => {
  //   setAlternateFormValue((prev) => prev.filter((_, ind) => ind !== index));
  //   setAlternateRoomDetails((prev) => prev.filter((_, ind) => ind !== index));
  //   setAlternateMealDetails((prev) => prev.filter((_, ind) => ind !== index));
  //   setHotelCheckBox((prev) => ({
  //     ...prev,
  //     copy: prev.copy?.filter((_, ind) => ind !== index) || [],
  //   }));
  //   setIsOpen((prev) => ({
  //     ...prev,
  //     copy: prev.copy?.filter((_, ind) => ind !== index) || [],
  ...
  function removeHotelAlternate() {
    setHotelFormValue([]);
    setHotelCheckBox({ original: [] });
    setRoomDetails([]);
    setMealDetails([]);
    setIsOpen({ original: false });
  }
  ...
  const handleHotelCopy = (e) => {
    // if (!mainHotelCheckBox) return;
    console.log(mainHotelCopyData.HotelForm, "HY98", hotelFormValue);
    const { checked } = e.target;
    setIsCopyHotel(true);

    if (checked && mainHotelCopyData) {
      setIsCopyHotel(true);

      // console.log(newHotel, "HY98", mainHotelCopyData.HotelForm);
      ...
      setHotelFormValue(mainHotelCopyData.HotelForm);
      setMealDetails(updatedMealType);
      setRoomDetails(updatedRoomType);

      dispatch(setItineraryCopyHotelFormDataCheckbox(false));
    } else {
      setIsCopyHotel(false);
      setHotelFormValue(localHotelValue?.HotelForm);
      setMealDetails(localHotelValue?.MealType);
