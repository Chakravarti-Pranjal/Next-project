...
const Activity = ({ Type, checkBoxes, headerDropdown }) => {
  ...
  // console.log(qoutationDataLocal?.Days, "rateList3836");
  ...
  const formValueInitialization = () => {
    ...
    if (qoutationDataLocal?.Days) {
      // console.log(qoutationDataLocal?.Days, "qoutationDataLocal?.Days");

      if (hasMonumentService) {
        const initialFormValue = qoutationDataLocal?.Days?.flatMap(
          (day, index, daysArray) => {
        ...
        qoutationDataLocal?.Days?.forEach((day, dayIndex) => {
  ...
  useEffect(() => {
    ...
    if (
      qoutationDataLocal?.Days?.length > 0 &&
      checkBoxes?.includes("activity") &&
      activityList?.length > 0 &&
      supplierList?.length > 0 &&
      allSuppliersLoaded
      ...
      const allDaysEmpty =
        Array.isArray(qoutationDataLocal?.Days) &&
        qoutationDataLocal?.Days.every((day) => {
