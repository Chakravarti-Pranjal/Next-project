...
import styles from "../itinerary-activity/index.module.css";
import { setItineraryCopyActivityFormDataCheckbox } from "../../../../../store/actions/itineraryServiceCopyAction/itineraryServiceCopyAction";
...
const LocalEscortActivityForm = ({ Type, checkBoxes, headerDropdown }) => {
  ...
  const formValueInitialization = () => {
    if (qoutationDataLocal?.Days) {
      ...
      if (hasMonumentService) {
        ...
        const activityInitialValue = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
  ...
  const setFirstValueIntoForm = (index) => {
    ...
    // const activityId = activityList?.[index]?.[0]?.id ?? "";
    // const supplier = supplierList?.[index]?.[0]?.id ?? "";
    const rate = rateList?.[index];

    // console.log(activityFormValue)
  ...
  useEffect(() => {
    const allRatesReady =
      ...
      rateList.length === qoutationDataLocal?.Days?.length &&
      rateList.every((r) => r !== undefined && r !== null);

    if (allRatesReady) {
      activityFormValue?.forEach((_, index) => {
    ...
    // activityFormValue.map((item) => item?.SupplierId).join(","),
    // isRateMerged,
  ]);
  ...
  // console.log(activityFormValue, "activityFormValue[index]?.ServiceId");
  ...
  //   console.log(activtiyUID?.UniqueId, "destination");
  ...
  //       ValidFrom: queryDataFromLocalStroage?.TravelDateInfo?.ScheduleType == "Date Wise" ? date : queryData?.QueryAllData?.TravelDateInfo?.FromDate,
  //       ValidTo: queryDataFromLocalStroage?.TravelDateInfo?.ScheduleType == "Date Wise" ? date : queryData?.QueryAllData?.TravelDateInfo?.ToDate,
  //       TotalActivity: "",
  ...
  //     if (data?.Data?.[0]?.TotalRecords.length > 0) {
  //       // console.log(data?.Data?.[0]?.RateJson?.ServiceCost,"data12")
  //       setRateList((prevArr) => {
  //         const updated = [...prevArr];
  ...
  //         return updated;
  //       });
  //     } else if (data?.Data?.[0]?.TotalRecords.length == 0) {
  //       // console.log(data?.Data?.[0]?.RateJson?.ServiceCost,"data12")
  //       setRateList([]);
  //     }
  ...
  useEffect(() => {
    ...
    activityFormValue?.forEach((form, index) => {
      // console.log(activityFormValue,"activityFormValue22");
      ...
      return {
        ...
        Cost:
          item?.Cost && !isNaN(item?.Cost)
            ? Math.floor(
                parseFloat(item?.Cost) + (parseFloat(item?.Cost) * hike) / 100
              )
            : item?.Cost,
      };
    });
  ...
  const calculateTotalAmount = (rowsMap) => {
    ...
    Object.entries(rowsMap).forEach(([day, rows]) => {
      ...
      totals[day] = {
        ActivityCost: mathRoundHelper(totalAmount),
    ...
    if (checked) {
      ...
      const updatedAdditional = mainFormValue?.ActivityForm?.map(
        (sourceActivity, index) => {
          ...
          console.log(formActivity, "formActivity4646");
          return {
            ...sourceActivity,
            DayType: "Local",
            DayUniqueId: formActivity?.DayUniqueId,
            Supplier: sourceActivity?.Supplier,
          };
        }
      );
      console.log(updatedAdditional, "updatedAdditional");
  ...
  // console.log(mainFormValue, "KKJ77");
              ...
              <table class="table table-bordered itinerary-table">
                ...
                <tbody>
                  ...
                  {/* <tr className="costing-td">
                    ...
                    <td colSpan={3}>{mathRoundHelper(totalAmount)}</td>
                  </tr>
                  <tr className="costing-td">
                    <td colSpan={3}>Markup(5) %</td>
                    <td colSpan={3}>{mathRoundHelper((totalAmount * 5) / 100)}</td>
                  </tr>
                  <tr className="costing-td">
                    <td colSpan={3}>Total</td>
                    <td colSpan={3}>{mathRoundHelper(totalAmount + (totalAmount * 5) / 100)}</td>
                  </tr> */}
                </tbody>
              </table>
            </div>
...
export default React.memo(LocalEscortActivityForm);
