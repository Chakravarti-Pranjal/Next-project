...
const LocalTransportFormValue = ({
  ...
  console.log(destinationList, "DEST456");
    ...
    if (name == "TransferType") {
      ...
      const DestinationId =
        value == 6 || value == 2
          ? Number(fromDestination)
          : Number(ToDestination);
      // console.log("HYT69", transportFormValue);
  ...
  console.log(dayUniqueIds, "dayUniqueIds452");
  ...
  useEffect(() => {
    const calculateTotalCostWithMarkup = (
      ...
      originalForm?.forEach((dayVehicles) => {
        ...
        console.log(dayVehicles, "dayVehicles");
      });
  ...
  //   setCopyTransportFormValue(transportFormValue);
  //   if (checked) {
  //     const updatedAdditional = transportFormValue?.map((service, index) => {
  //       return {
  ...
  //     setOriginalVehicleTypeForm(transportData?.FormattedVehicleList);

  //     setCopyChecked(true);

  //     dispatch(setItineraryCopyTransportFormDataCheckbox({ local: false }));
  //   } else {
  ...
  const handleCopyDataFromMain = async (e) => {
    ...
    //   const hasTransportServices = quotationData?.Days?.some((day) => {
    //     const transportServices = day?.DayServices?.filter(
    //       (service) =>
    //         service?.ServiceType === "Transport" &&
    //         service.ServiceMainType === "Guest"
    //     );
    //     return transportServices?.length > 0;
    //   });
    //   if (!hasTransportServices) {
    //   // setTransportFormValue(defaultTransportFormValue);
    //   // setCopyTransportFormValue(defaultTransportFormValue);
    //   // setCopyChecked(false);
    //   // setTransportList([]);
    //   // setTransportSupplierList([]);
    ...
    // console.log(quotationData?.Days,"quotationdata11");
    ...
    quotationData?.Days?.forEach((day, ind) => {
      console.log(day, ind, "day?.DayServices");
      ...
      console.log(transportServices, "transportServices");
      ...
      transportServices?.forEach((service) => {
        ...
        // Ensure TransferTypeId is parsed as an integer
        const transferTypeId = service?.TransferTypeId
          ? parseInt(service.TransferTypeId)
          : "";

        // Push to form value
        // console.log(transferTypeId,"transferTypeId");
        // console.log(travelDataEntry,"travelDataEntry");
        // console.log(dayTravelData,"dayTravelData");
        // console.log();
        ...
        initialFormValue.push({
          ...
          FromDestination: service?.FromDestinationId || day?.DestinationId,
          ToDestination: service?.ToDestinationId,
          TransferType: transferTypeId, // Ensure this is set
          Mode: service?.Mode || travelDataEntry?.Mode || "",
          Escort: 1,
          Supplier: ItemSupplierDetail?.ItemSupplierId || "",
          TransportDetails: service?.TransportDetails || "",
          ...
          AlternateVehicleCost: service?.AlternateVehicleCost || "",
          NoOfDay: service?.NoOfDay || "",
          ServiceId: service?.ServiceId || "", // Preserve ServiceId
          ItemFromDate: TimingDetails?.ItemFromDate || "",
          ItemToDate: TimingDetails?.ItemToDate || "",
          ItemFromTime: TimingDetails?.ItemFromTime || "",
          ItemToTime: TimingDetails?.ItemToTime || "",
    ...
    console.log("initialFormValue:", initialFormValue); // Debug initial form value

    // Step 2: Update transportFormValue and copyTransportFormValue
    setTransportFormValue(initialFormValue);
    setCopyTransportFormValue(initialFormValue);

    console.log("transportFormValue after set:", initialFormValue); // Debug after setting
    ...
    if (checked) {
      const updatedAdditional = initialFormValue?.map((service, index) => {
        ...
        const { DayUniqueId, ...restTransportData } = transportDataItem; // Explicitly exclude DayUniqueId
        return {
          ...restTransportData,
          DayType: "Local",
          TransferType:
      ...
      console.log("updatedAdditional:", updatedAdditional); // Debug merged data

      // Step 4: Fetch transportList for dropdowns

      const fetchTransportLists = async () => {
        const allTransportLists = [];
        const allSupplierLists = [];
        ...
        for (let index = 0; index < updatedAdditional.length; index++) {
          ...
          try {
            ...
            // Fetch supplier list (optional, for consistency)
        ...
        // Update transportList and supplierList for dropdowns
        setTransportList(allTransportLists);
        setTransportSupplierList(allSupplierLists);
      };

      await fetchTransportLists();

      // Update transportFormValue with merged data (without modifying ServiceId yet)
      setTransportFormValue(updatedAdditional);
      // const transportData = useSelector(
      //   (state) => state.itineraryServiceCopyReducer.transportData
      // );
      ...
      // console.log("transportData?.FormattedVehicleList:", transportData?.FormattedVehicleList);
      // console.log("transportData?.SelectedOptionVehicle:", transportData?.SelectedOptionVehicle);
      // console.log("formattedVehicles:", formattedVehicles);

      // Set selectedOptionSecond with priority to SelectedOptionVehicle
      setSelectedOptionSecond(
      ...
      console.log(
        transportData?.FormattedVehicleList,
        "transportData?.FormattedVehicleList"
      );
      ...
      setCopyChecked(false);
      // setTransportFormValue(initialFormValue);
      // setTransportList([]); // Clear transportList
      // setTransportSupplierList([]); // Clear supplierList
      // setSelectedOptionSecond([]);
      // setFormattedVehicleList([]);
      // setOriginalVehicleTypeForm([]);
    }

    // Trigger useEffect to re-render dropdowns
    setData((prev) => !prev);

    console.log("transportFormValue after render:", transportFormValue); // Debug final state
  };
                        ...
                        formattedVehicleList[0]?.map((options, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {console.log(calculatedRateDetails, "checkdata")}
                            </td>
                          );
                        })}
                    </tr>
                    ...
                    <tbody>
                      ...
                      <tr className="costing-td">
                        ...
                        {selectedOptions?.map((options, index) => {
                          console.log(options, "Options2580");
                          return (
                            <td key={index}>
                              {mathRoundHelper(
...
export default React.memo(LocalTransportFormValue);
