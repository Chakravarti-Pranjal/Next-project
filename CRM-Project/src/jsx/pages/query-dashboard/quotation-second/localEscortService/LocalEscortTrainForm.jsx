...
const LocalEscortTrainForm = ({ Type, transportFormValue }) => {
  ...
  useEffect(() => {
    if (rateList?.length == qoutationDataLocal?.Days?.length && isRateMerge) {
      mergeTrainRateJson();
    }
  }, [
    rateList,
  ...
  //   // Apply hike on rateList values if available, else on originalTrainForm
  //   const updatedData = originalTrainForm?.map((item, idx) => {
  //     // Find rate from rateList
  ...
  //     // Use rate values if available, else fallback to item values
  //     const adultCost = rate.AdultCost !== undefined ? parseFloat(rate.AdultCost) : parseFloat(item.AdultCost) || 0;
  //     const childCost = rate.ChildCost !== undefined ? parseFloat(rate.ChildCost) : parseFloat(item.ChildCost) || 0;
  //     const guideCharges = rate.AdultCost !== undefined ? parseFloat(rate.AdultCost) : parseFloat(item.GuideCharges) || 0;
  //     const serviceCharges = rate.AdultCost !== undefined ? parseFloat(rate.AdultCost) : parseFloat(item.ServiceCharges) || 0;
  //     const handlingCharges = rate.ChildCost !== undefined ? parseFloat(rate.ChildCost) : parseFloat(item.HandlingCharges) || 0;

  //     return {
  //       ...item,
  //       Hike: value,
  //       AdultCost: Math.floor(adultCost + (adultCost * value) / 100),
  //       ChildCost: Math.floor(childCost + (childCost * value) / 100),
  //       GuideCharges: Math.floor(guideCharges + (guideCharges * value) / 100),
  //       ServiceCharges: Math.floor(serviceCharges + (serviceCharges * value) / 100),
  //       HandlingCharges: Math.floor(handlingCharges + (handlingCharges * value) / 100),
  //     };
  //   });
  //   setTrainFormValue(updatedData);
  // };
  ...
  const handleHikeChange = (e) => {
    ...
    if (!value || isNaN(hikePercent)) {
      // 🟢 Reset to original data if hike is empty or invalid
  ...
  const handleCopy = (e) => {
    // if (!trainCheckbox.foreigner) return;
    const { checked } = e.target;

    console.log(trainData, "trainData7464 handlecopycalled");
    ...
    if (checked) {
      ...
      setCopyChecked(true);
      dispatch(setItineraryCopyTrainFormDataCheckbox({ local: false }));
    } else {
      setCopyChecked(false);
    }
  };

  console.log(trainFormValue, "WQTSFSTTSTS");
...
export default LocalEscortTrainForm;
