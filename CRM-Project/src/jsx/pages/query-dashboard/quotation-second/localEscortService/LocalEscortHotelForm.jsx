...
import { setItineraryCopyHotelFormDataCheckbox } from "../../../../../store/actions/itineraryServiceCopyAction/itineraryServiceCopyAction";
...
import HotelDetailModel from "../itinerary-hotel/HotelDetailModel";
...
const LocalEscortHotelForm = ({ headerDropdown, Type, ActiveOptionId }) => {
  ...
  console.log("Qutation-date", hotelFormValue);
  ...
  console.log(qoutationDataLocal, "qoutationDataLocal785");
  ...
  useEffect(() => {
    if (qoutationDataLocal?.Days) {
      ...
      if (hasMonumentService) {
        // setIsCopyHotel(true)
        const initialFormValue = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
        ...
        console.log(initialFormValue, "INITIAL896");
        setHotelFormValue(initialFormValue);

        const servicesRoom = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        const staticRoom = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        const servicesMeal = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        const staticMeal = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        console.log(qoutationDataLocal.Days, "QOUT652");
        const initialFormValue = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
        ...
        console.log("HHY78Local", initialFormValue);
        setHotelFormValue(initialFormValue);
        setFormIsEmpty(true);

        // creating room details array
        const roomJson = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
        ...
        const mealJson = qoutationDataLocal?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
  ...
  console.log(hotelFormValue, "HoTe25");
  ...
  const getHotelRateApi = async (
    ...
    try {
      console.log(mealPlan, "mealPlan900");
      ...
      if (hotel && companyId && destination) {
        const { data } = await axiosOther.post("priceEditHotelRatesJson", {
          ...
          RoomTypeId: "",
          MarketTypeId: "",
          HotelCategoryId: HotelCategoryvalue || "",
          SupplierID: " ",
          PaxTypeId: "",
          ValidFrom: "",
          ValidTo: " ",
        ...
        if (data?.Status === 1 && data?.Data?.length > 0) {
          ...
          setHotelFormValue((prevValue) => {
            ...
            newArr[index] = {
              ...
              ServiceId: HotelCategory || hotelFormValue[index]?.ServiceId,

              RoomCategory: rateData?.RateJson?.RoomTypeId,
            };
            return newArr;
          });
  ...
  const getHotelListDependently = async (
    ...
    try {
      ...
      setHotelFullList((prev) => {
        const newList = [...prev];
        if (!hotelname) newList[index] = data?.DataList || [];
        return newList;
      });
  ...
  //   // headerDropdown.Hotel = hotelForm?.HotelCategory;

  //   if (["HotelCategory"].includes(field)) {
  ...
  //       index,
  //       ""
  //       // ✅ pass full object, not .hotelData
  //     );
  //   }
  // }, [selectedInd]);
  ...
  // ============================================
  console.log(hotelFormValue, "hotelFormValuejkj");
  ...
  console.log(mainHotelCopyData.HotelForm, "HOT452");

  const handleHotelCopy = (e) => {
    // if (!mainHotelCheckBox.local) return;
    console.log("HY98", hotelFormValue);
    const { checked } = e.target;
    if (checked) {
      const newHotel = mainHotelCopyData?.HotelForm?.map((mainHotel) => {
        // find a matching entry in hotelFormValue with same DayNo
        const matchedHotel = hotelFormValue?.find(
          (h) => h?.DayNo === mainHotel?.DayNo
        );

        return {
          ...mainHotel,
          DayType: "Local",
          DayUniqueId: matchedHotel?.DayUniqueId,
        };
      });

      // console.log(hotelFormValue, 'hotelFormVAluexyz')
      console.log(newHotel, "HY98");

      const updatedMealType = JSON.parse(
        JSON.stringify(mainHotelCopyData?.MealType)
      );
      const updatedRoomType = JSON.parse(
        JSON.stringify(mainHotelCopyData?.RoomBedType)
      );

      setHotelFormValue(newHotel);
      setMealDetails(updatedMealType);
      setRoomDetails(updatedRoomType);
      setIsCopyHotel(true);
      dispatch(setItineraryCopyHotelFormDataCheckbox({ local: false }));
    } else {
                                ...
                                value={
                                  hotelList[index]
                                    ...
                                    .find(
                                      (option) =>
                                        option.value ===
                                        hotelFormValue[index]?.ServiceId
                                    ) || null
                                }
                                onChange={(selected) =>
                    ...
                    <tr className="costing-td">
                      ...
                      {mealDetails[0]?.map((meal, index) => {
                        ...
                        return (
                          hotelCheckBox?.original?.includes(
                            meal?.MealTypeId
                          ) && <td>{mathRoundHelper(rate?.MealCost) || 0}</td>
                        );
                      })}
                    </tr>
                    <tr className="costing-td">
                    ...
                    <table class="table table-bordered itinerary-table">
                      <thead>
                        <tr className="its-table">
                          ...
                          >
                            {alternate?.Form?.[0]?.Date ? "Day / Date" : "Day"}
                          </th>
                          {(Type == "Local" || Type == "Foreigner") && (
                            <th rowSpan={2} className="py-1 align-middle">
                              Escort
                              ...
                              <td>
                                <div>
                                  <select
                                    ...
                                    id=""
                                    className={`formControl1 ${
                                      isFocus == hotelIndex + "b" && "focus-red"
                                    }`}
                                    // value={
                                    //   alternateFormValue[index][hotelIndex]
                                    //     ?.Destination
                              ...
                              <td>
                                <div>
                                  <select
                                    ...
                                    id=""
                                    className={`formControl1 ${
                                      isFocus == hotelIndex + "e" && "focus-red"
                                    }`}
                                    value={
                                      alternateFormValue[index]?.Form[
                                        hotelIndex
                        ...
                        <tr className="costing-td">
                          ...
                          {hotelCheckBox?.copy[index]?.includes("extrabed")
                            ? roomDetails[0]?.map((room, index) => {
                                ...
                                return <td key={index}>{rate?.RoomCost}</td>;
                              })
                            : roomDetails[0]?.map((room, index) => {
                                const rate =
                                  calculatedRateDetails?.RoomRates?.find(
                        ...
                        <tr className="costing-td">
                          ...
                          {hotelCheckBox?.copy[index]?.includes("extrabed")
                            ? roomDetails[0]?.map((room, index) => {
                                ...
                                return <td key={index}>{rate?.Markup}</td>;
                              })
                            : roomDetails[0]?.map((room, index) => {
                                const rate =
                                  calculatedRateDetails?.RoomRates?.find(
...
export default LocalEscortHotelForm;
