...
const Activity = ({ Type, checkBoxes, headerDropdown }) => {
  ...
  // console.log(qoutationDataForeigner?.Days, "rateList3836");
  const hasMonumentService = qoutationDataForeigner?.Days?.some((day) =>
  ...
  const formValueInitialization = () => {
    ...
    if (qoutationDataForeigner?.Days) {
      // console.log(qoutationDataForeigner?.Days, "qoutationDataForeigner?.Days");

      if (hasMonumentService) {
        const initialFormValue = qoutationDataForeigner?.Days?.flatMap(
          (day, index, daysArray) => {
        ...
        qoutationDataForeigner?.Days?.forEach((day, dayIndex) => {
        ...
        console.log(
          qoutationDataForeigner?.Days,
          "qoutationDataForeigner?.Days"
        );
        const activityInitialValue = qoutationDataForeigner?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
  ...
  useEffect(() => {
    ...
    if (
      qoutationDataForeigner?.Days?.length > 0 &&
      checkBoxes?.includes("activity") &&
      activityList?.length > 0 &&
      supplierList?.length > 0 &&
      allSuppliersLoaded
      ...
      const allDaysEmpty =
        Array.isArray(qoutationDataForeigner?.Days) &&
        qoutationDataForeigner?.Days.every((day) => {
  ...
  useEffect(() => {
    activityFormValue.forEach((activity, idx) => {
      ...
      if (ServiceId && DestinationUniqueId && !hasMonumentService) {
