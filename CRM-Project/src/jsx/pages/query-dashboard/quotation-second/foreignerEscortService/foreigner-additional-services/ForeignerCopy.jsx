...
const ForeignerAdditionalServices = ({ Type }) => {
  ...
  useEffect(() => {
    if (qoutationDataForeigner?.Days) {
      const hasAdditionalService = qoutationDataForeigner?.Days.some((day) =>
      ...
      if (hasAdditionalService) {
        // console.log("Additional Service: ", qoutationData);

        const initialFormValue = qoutationDataForeigner?.Days?.map(
          (day, index) => {
        ...
        const additionalInitialValue = qoutationDataForeigner?.Days?.map(
          (day, ind) => {
            return {
  ...
  const handleFinalSave = async () => {
    try {
      const finalForm = additionaFormValue?.map((form, index) => {
        ...
        console.log(dayWiseTotalCost, "dayWiseTotalCost");
  ...
  const applyHike = () => {
    ...
    // console.log(hikePercent,"hikePercent");
    // console.log(rowsPerIndex,"rowsPerIndex");

    const list = rowsPerIndex[selectedIndex?.toString()];
    if (Array.isArray(list)) {
      const updatedData = list.map((item) => {
        ...
        return {
          ...item,
          upTo: Math.floor(upTo + (upTo * hike) / 100),
          rounds: Math.floor(rounds + (rounds * hike) / 100),
          class: Math.floor(rounds + (classes * hike) / 100),
          duration: Math.floor(duration + (duration * hike) / 100),
          amount: Math.floor(amount + (amount * hike) / 100),
        };
      });

      setRowsPerIndex((prev) => ({
        ...prev,
        [selectedIndex]: updatedData,
      }));
    } else {
      console.log("Expected an array but got:", list);
    }
  };
  ...
  useEffect(() => {
    if (Type !== "Main" && additionalFormData?.length > 0) {
      if (copyChecked) {
        // Only backup if backupAdditionalForms is empty to avoid overwriting
        if (backupAdditionalForms.length === 0) {
          // console.log("Backing up additionaFormValue:", additionaFormValue);
          setBackupAdditionalForms(additionaFormValue);
        }
        // console.log(
        //   "Copying additionalFormData to additionaFormValue:",
        //   additionalFormData
        // );
        setAdditionalFormValue(additionalFormData);
        setOriginalAdditionalForm(additionalFormData);
      } else {
        // Restore only if backupAdditionalForms has data
        if (backupAdditionalForms.length > 0) {
          // console.log(
          //   "Restoring from backupAdditionalForms:",
          //   backupAdditionalForms
          // );
          setAdditionalFormValue(backupAdditionalForms);
          setOriginalAdditionalForm(backupAdditionalForms);
          setBackupAdditionalForms([]); // Clear backup after restoration
        } else {
          console.log("No backup data to restore");
        }
      }
    }
  }, [copyChecked, Type, backupAdditionalForms, additionaFormValue]);
...
export default ForeignerAdditionalServices;
