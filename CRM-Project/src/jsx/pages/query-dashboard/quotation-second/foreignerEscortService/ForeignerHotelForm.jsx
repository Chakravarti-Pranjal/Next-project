...
const ForeignerHotelForm = ({ headerDropdown, Type, ActiveOptionId }) => {
  ...
  useEffect(() => {
    if (qoutationDataLocal?.Days) {
      ...
      if (hasMonumentService) {
        ...
        // Call handleHotelCopy with checked: true on first visit
        handleHotelCopy({ target: { checked: true } });
  ...
  const handleHotelCopy = (e) => {
    const { checked } = e.target;
    console.log(checked, "KKJKJ");
    if (checked) {
      const foreignerDays = qoutationDataLocal?.Days?.filter(
        (day, index, daysArray) => {
          const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
          if (
            previousEnrouteId != null &&
      ...
      const newHotel = mainHotelCopyData?.HotelForm?.map((mainHotel) => {
        // Find a matching day in qoutationDataLocal.Days by DayNo
        const matchedDay = foreignerDays?.find(
          (day) => day?.Day === mainHotel?.DayNo
        );
        return {
          ...mainHotel,
          DayType: "Foreigner",
          DayUniqueId: matchedDay?.DayUniqueId || mainHotel?.DayUniqueId, // Use foreigner DayUniqueId if available
        };
      });
      ...
      setOriginalMealDetails(updatedMealType);
      setRoomDetails(updatedRoomType);
      setOriginalRoomDetails(updatedRoomType);
      setIsCopyHotel(true);
      dispatch(setItineraryCopyHotelFormDataCheckbox({ local: false }));
    } else {
      setIsCopyHotel(false);
      // Reset to initial foreigner form values
      // const initialFormValue = qoutationDataLocal?.Days?.filter(
      //   (day, index, daysArray) => {
      //     const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
      //     if (
      ...
      //     Escort: "",
      //   },
      // }))?.filter((item) => !item.EnrouteId);
      // setHotelFormValue(initialFormValue);
      // // Reset meal and room details to initial empty state
      // const roomJson = qoutationDataLocal?.Days?.filter(
      //   (day, index, daysArray) => {
      //     const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
      //     if (
...
export default ForeignerHotelForm;
