...
import usePaxSlabForm from "./pax-slab-tab/PaxSlabFormData";
...
const Commission = ({ onNext, onBack }) => {
  ...
  const slabData = usePaxSlabForm();
  // console.log(slabData, "VCBHD8777");
  ...
  const calculateMargins = (slab) => {
    ...
    Object.keys(typeMapping).forEach((field) => {
      ...
      // console.log(value, "WTTSGTS", field);
  ...
  useEffect(() => {
    if (qoutationData?.PaxSlab?.length > 0) {
      ...
      const initialFormValue = qoutationData?.PaxSlab?.map((pax, index) => {
        ...
        // console.log(formValue.LocalEscort, "DFSGSVSG");
        return {
          ...formValue,
  ...
  useEffect(() => {
    if (qoutationData && Object.keys(qoutationData).length > 0) {
      const GstType = dropdownGSTType?.[0]?.id;
      const Gst = gstTax?.[0]?.id;
      ...
      // console.log(GstType, "MarkupType");
      // console.log(dropdownGSTType, "BCVGDYDTGD8777");
  ...
  }, [qoutationData, gstTax]);

  useEffect(() => {
    const updatedPolicyData = Object.keys(policyData);
    // console.log(policyData, "YSTTDGDGDGTTD98");
    if (policyData && updatedPolicyData.length > 0) {
      setFormValue((prevFormValue) => ({
        ...prevFormValue,
        FitId: policyData?.id,
        OverviewId: policyData?.Overview || "",
        OverviewIncExcTc: {
          TourHeighligts: policyData?.TourHighlight || "",
          Overview: policyData?.OverviewName || "",
          ItineraryIntroduction: policyData?.ItineraryIntroduction || "",
          ItinerarySummary: policyData?.ItinerarySummary || "",
        },
        FitIncExc: {
          Inclusion: policyData?.Exclusion || "",
          Exclusion: policyData?.Inclusion || "",
          TermsNCondition: policyData?.TermsNCondition || "",
          CancellationPolicy: policyData?.CancellationPolicy || "",
          PaymentPolicy: policyData?.BookingPolicy || "",
  ...
  const handleSubmit = async () => {
    ...
    const lengthData = qoutationData?.PaxSlab?.length;
    try {
      // try {
        ...
        <div className="px-5 mt-1">
          <div className="row mt-3 ml-1 row-gap-2 m-0">
            <div className="col-6 col-sm-4 col-md-3 col-lg-2 p-1">
              <table className="border table-responsive w-100">
                <thead>
                  <tr>
                  ...
                  <tr>
                    <td className="p-0 text-center py-2 px-3">
                      <select
                        ...
                        id=""
                        style={{ width: "100px" }}
                        className="px-1 table-select-1 text-truncate font-size10 formControl1"
            ...
            <div className="col-6 col-sm-4 col-md-3 col-lg-2 p-1">
              <table className="border table-responsive w-100">
                ...
                {/* {console.log(formValue?.OthersInfo?.Gst, "VCYSR&666", gstTax)} */}
                <tbody>
                  <tr>
                    <td className="p-0 text-center py-2">
                      <select
                  ...
                  <tr>
                    <td className="p-0 text-center py-2 px-2">
                      <select
                        ...
                        onChange={hanldeOtherInfo}
                        value={formValue?.OthersInfo?.SrsandTrr || "SRS"}
                      >
