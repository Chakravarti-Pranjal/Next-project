...
const AdditionalUpgrade = ({ Type }) => {
  ...
  useEffect(() => {
    if (qoutationData?.Days) {
      ...
      if (hasTrainService) {
        const initialFormValue = qoutationData?.Days?.map((day, index) => {
          const service = day?.DayServices?.filter(
            (service) =>
              service?.ServiceType == "Additional" &&
              service.ServiceMainType === "Additional Suppliment"
          )[0];
  ...
  console.log(upgradeAdditionalFormData, "upgradeAdditionalFormDatakdkd");
  ...
  const handleHikeChange = (e) => {
    ...
    const updatedData = originalAdditionalForm?.map((item) => {
      return {
        ...item,
        Hike: value,
        AdultCost:
    ...
    setAdditionalFormValue(updatedData);
  };
  ...
  const calculateTotalAmount = (rowsMap) => {
    ...
    Object.entries(rowsMap).forEach(([day, rows]) => {
      ...
      totals[day] = {
        ServiceAdultCost: totalAmount,
        ServiceChildCost: 0,
        AdultMarkupValue: adultMarkupValue,
        ChildMarkupValue: 5,
        AdultMarkupTotal: adultMarkupTotal,
        ChildMarkupTotal: 0,
        TotalAdultServiceCost: totalAmount + adultMarkupTotal,
  ...
  const handleCopyDataFromMain = (e) => {
    ...
    if (checked !== false) {
      if (checked && upgradeAdditionalFormData) {
        // const updatedAdditional = additionaFormValue?.map((service, index) => {
        //   return {
        //     ...additionalMainData.AdditionalForm[index],
        //     DayType: "Local",
        ...
        setCopyChecked(true);
        setAdditionalFormValue(upgradeAdditionalFormData?.AdditionalForm);
        const updatedCosting = JSON.parse(
          JSON.stringify(upgradeAdditionalFormData?.Costing)
        );
        setRowsPerIndex(updatedCosting);
        // dispatch(setItineraryCopyAdditionalFormDataCheckbox(false));
      } else {
        setCopyChecked(false);
        setAdditionalFormValue(additionaFormValue);
        setRowsPerIndex(rowsPerIndex);
      }
    }
  };
        ...
        <div className="d-flex gap-4 align-items-center">
          ...
          >
            <input
              ...
              id="copy-guide"
              checked={copyChecked}
              onChange={handleCopyDataFromMain}
            />
            ...
            <div className={`${styles.scrollContainer}`}>
              <table class="table table-bordered itinerary-table">
                ...
                <tbody>
                  ...
                  <tr className="costing-td">
                    <td>Markup(5) %</td>
                    <td>{(totalAmount * 5) / 100}</td>
                  </tr>
                  <tr className="costing-td">
                    <td>Total</td>
                    <td>{totalAmount + (totalAmount * 5) / 100}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
export default AdditionalUpgrade;
