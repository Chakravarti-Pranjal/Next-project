...
const Flight = ({ transportFormValue, Type, TabId }) => {
  ...
  //   // console.log(airlineTypeId,"checkfirstt2")
  ...
  // useEffect(() => {
  //   if (isMergeRate && !hasTrainService) {
  //     mergeFlightRateJson();
  //   }
  ...
  const handleFinalSave = async () => {
    ...
    try {
      ...
      const { data } = await axiosOther.post(
        "update-multiple-quatation-flight",
        // filteredFinalForm
        updatedJsonData
      );
      if (data?.status == 1) {
  ...
  //   if (checked) {
  //     let newUpdateAdditional = flightFormValue?.map((item, index) => ({
  //       ...flightData.FlightFormValue[index],
  //       QueryId: queryData?.QueryAlphaNumId,
  //       OptionId: TabId,
  //       QuatationNo: OptionQoutationData?.QuotationNumber,
  //     }));
  //     setFlightFormValue(newUpdateAdditional);
  //     setCopyChecked(true);
  //   }
  // };
              ...
              <table class="table table-bordered itinerary-table">
                ...
                <tbody>
                  ...
                  <tr className="costing-td">
                    ...
                    <td>Flight Cost</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.Price?.Adult)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.Price?.Child)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.Price?.Guide)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.Price?.Service)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.Price?.Handling)}</td>
                    <td></td>
                  </tr>
                  <tr className="costing-td">
                    <td>Markup(5) %</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.MarkupOfCost?.Adult)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.MarkupOfCost?.Child)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.MarkupOfCost?.Guide)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.MarkupOfCost?.Service)}</td>
                    <td>{mathRoundHelper(flightPriceCalculation?.MarkupOfCost?.Handling)}</td>
                    <td></td>
                  </tr>
                  <tr className="costing-td">
                    <td>Total</td>
                    <td>
                      {mathRoundHelper(parseInt(flightPriceCalculation?.Price?.Adult) +
                        parseInt(flightPriceCalculation?.MarkupOfCost?.Adult))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(flightPriceCalculation?.Price?.Child) +
                        parseInt(flightPriceCalculation?.MarkupOfCost?.Child))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(flightPriceCalculation?.Price?.Guide) +
                        parseInt(flightPriceCalculation?.MarkupOfCost?.Guide))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(flightPriceCalculation?.Price?.Service) +
                        parseInt(flightPriceCalculation?.MarkupOfCost?.Service))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(flightPriceCalculation?.Price?.Handling) +
                        parseInt(
