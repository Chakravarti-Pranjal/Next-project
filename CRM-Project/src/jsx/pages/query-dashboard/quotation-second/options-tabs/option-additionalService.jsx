...
const Additional = ({ Type, TabId }) => {
  ...
  useEffect(() => {
    if (OptionQoutationData?.Days) {
      ...
      if (hasTrainService) {
        const initialFormValue = OptionQoutationData?.Days?.map(
          (day, index) => {
            const service = day?.DayServices?.filter(
              (service) => service?.ServiceType == "Additional"
            )[0];

            console.log(service, "ADDITIONAL8777");
        ...
        const additionalInitialValue = OptionQoutationData?.Days?.map(
          (day, ind) => {
            return {
  ...
  const handleFinalSave = async () => {
    try {
      ...
      const { data } = await axiosOther.post(
        "update-multiple-quatation-additional",
        FilteredfinalForm
      );
  ...
  useEffect(() => {
    if (additionaFormValue.length > 0) {
      const exists = selectedIndex !== null && additionaFormValue[selectedIndex] !== undefined;
  ...
  //   if (checked) {
  //     let newUpdateAdditional = additionaFormValue?.map((item, index) => ({
  ...
  //     setAdditionalFormValue(newUpdateAdditional);
