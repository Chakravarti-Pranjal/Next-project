...
import "../itinerary-train/index.css";
...
const Trian = ({ Type, transportFormValue, TabId }) => {
  ...
  // );
  const hasTrainService = OptionQoutationData?.Days?.some((day) =>
    day?.DayServices?.some((service) => service.ServiceType === "Train")
  );
  // console.log(hasTrainService, "hasTrainService")
  ...
  useEffect(() => {
    if (OptionQoutationData?.Days) {
      ...
      if (hasTrainService) {
        const initialFormValue = OptionQoutationData?.Days?.filter((day) => {
        ...
        const trainInitialValue = transportFormValue
          ...
          ?.map((transport) => {
            const day = OptionQoutationData?.Days?.find(
  ...
  }, [OptionQoutationData, transportFormValue, TabId]);
  ...
  const getSupplierDependentOnDestination = async (destination, index) => {
    try {
      ...
      // Only update Supplier if data is available
      if (data?.DataList?.length > 0) {
        setTrainFormValue((prev) => {
          const newArr = [...prev];
          newArr[index] = {
            ...newArr[index],
        ...
        // Optionally reset Supplier if no data is found
        setTrainFormValue((prev) => {
          const newArr = [...prev];
          newArr[index] = {
            ...newArr[index],
      ...
      // Handle error gracefully, e.g., reset Supplier
      setSupplierList((prevArr) => {
        const newArr = [...prevArr];
        newArr[index] = [];
        return newArr;
  ...
  // useEffect(() => {
  //   if (isRateMerge && !hasTrainService) {
  //      mergeTrainRateJson();
  //   }
  // }, [
  //   rateList,
  ...
  const handleFinalSave = async () => {
    ...
    try {
      ...
      const { data } = await axiosOther.post(
        "update-multiple-quatation-train",
        // filteredFinalJson
        updatedJsonData
      );
      if (data?.status == 1) {
  ...
  //   if (checked) {
  //     const newTrain = trainFormValue?.map((train, index) => {
  //       return {
  //         ...trainData.TrainFormValue[index],
  //         QueryId: queryData?.QueryAlphaNumId,
  //         OptionId: TabId,
  //         QuatationNo: OptionQoutationData?.QuotationNumber,
  //       };
              ...
              <table class="table table-bordered itinerary-table">
                ...
                <tbody>
                  ...
                  <tr className="costing-td">
                    ...
                    <td>Train Cost</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.Price?.Adult)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.Price?.Child)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.Price?.Guide)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.Price?.Service)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.Price?.Handling)}</td>
                    <td></td>
                  </tr>
                  <tr className="costing-td">
                    <td>Markup(5) %</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.MarkupOfCost?.Adult)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.MarkupOfCost?.Child)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.MarkupOfCost?.Guide)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.MarkupOfCost?.Service)}</td>
                    <td>{mathRoundHelper(trainPriceCalculation?.MarkupOfCost?.Handling)}</td>
                    <td></td>
                  </tr>
                  <tr className="costing-td">
                    <td>Total</td>
                    <td>
                      {mathRoundHelper(parseInt(trainPriceCalculation?.Price?.Adult) +
                        parseInt(trainPriceCalculation?.MarkupOfCost?.Adult))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(trainPriceCalculation?.Price?.Child) +
                        parseInt(trainPriceCalculation?.MarkupOfCost?.Child))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(trainPriceCalculation?.Price?.Guide) +
                        parseInt(trainPriceCalculation?.MarkupOfCost?.Guide))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(trainPriceCalculation?.Price?.Service) +
                        parseInt(trainPriceCalculation?.MarkupOfCost?.Service))}
                    </td>
                    <td>
                      {mathRoundHelper(parseInt(trainPriceCalculation?.Price?.Handling) +
                        parseInt(trainPriceCalculation?.MarkupOfCost?.Handling))}
                    </td>
                    <td></td>
                  </tr>
