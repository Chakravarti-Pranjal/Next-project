...
const Activity = ({ Type, headerDropdown, checkBoxes, TabId }) => {
  ...
  console.log(OptionQoutationData, "ACT877");
  ...
  const formValueInitialization = () => {
    if (OptionQoutationData?.Days) {
      if (hasMonumentService) {
        ...
        const activityInitialValue = OptionQoutationData?.Days?.filter(
          (day, index, daysArray) => {
            const previousEnrouteId = daysArray[index - 1]?.EnrouteId;
            if (
              previousEnrouteId != null &&
  ...
  }, [OptionQoutationData, checkBoxes, TabId]);
  ...
  }, [OptionQoutationData, checkBoxes, activityList, supplierList]);
  ...
  const handleFinalSave = async () => {
    try {
      ...
      const { data } = await axiosOther.post(
        "update-multiple-quatation-activity",
        finalForm
      );
  ...
  useEffect(() => {
    activityFormValue.forEach((activity, idx) => {
      const { ServiceId, DestinationUniqueId, Date } = activity || {};
      ...
      if (
        ...
        getActivityRateApi(DestinationUniqueId, idx, Date, ServiceId);
      }
    });
  ...
  const calculateTotalAmount = (rowsMap) => {
    ...
    Object.entries(rowsMap).forEach(([day, rows]) => {
      ...
      totals[day] = {
        ...
        TotalActivityCost: mathRoundHelper(String(totalAmount + adultMarkupTotal)),
      };
    });
                ...
                <table class="table table-bordered itinerary-table">
                  ...
                  <tbody>
                    ...
                    <tr className="costing-td">
                      ...
                      <td colSpan={3}>
                        {mathRoundHelper((totalAmount * ActivityData?.Value) / 100 || 0)}
                      </td>
                    </tr>
                    <tr className="costing-td">
                      <td colSpan={3}>Total</td>
                      <td colSpan={3}>
                        {mathRoundHelper(totalAmount +
