...
const Restaurant = ({ Type }) => {
  ...
  console.log(qoutationData, "qoutationData363636");
  ...
  useEffect(() => {
    ...
    if (qoutationData?.Days) {
      ...
      if (hasMonumentService) {
        ...
        qoutationData?.Days?.map((day) => {
          const serviceData = day?.DayServices?.filter(
            (service) => service?.ServiceType == "Restaurant"
          );
          ...
          for (const service of serviceData) {
            ...
            initialFormValue.push(serviceObject);
          }
        });
        ...
        const restaurantMeal = qoutationData?.Days?.map((day, index) => {
          // Use filter to get all services of type "Restaurant"
          const services =
            day?.DayServices?.filter((s) => s?.ServiceType === "Restaurant") ||
            [];

          // If no restaurant services are found, return an empty array
          if (!services.length) return [];

          // Map over each restaurant service to create meal plans
          const mealPlans = services.flatMap((service) => {
            ...
            console.log(service, "VCBH877");

            const mealObject =
              service?.MealPlan?.map((meal) => {
                console.log(meal, "VCHDGD877");

                return {
                  ...meal,
                  MealType: meal.MealType,
                  ServiceID: meal?.ServiceID,
                  ServiceType: meal?.ServiceType,
                  ServiceName: meal.ServiceName,
                  RestaurantId:
                    meal?.serviceType !== "Hotel" ? meal?.ServiceID : "",
            ...
            console.log(mealObject, "VCHHD*777");
            mealPlanList.push(mealObject);
            return mealObject;
          });
        ...
        console.log(restaurantMeal, "restaurantMeal74747");
        mealPlanList?.forEach((dayMeals, dayIndex) => {
        ...
        console.log(initialFormValue, "initialFormValue64545");

        setCheckSupplement(initialCheckedMeals);
        setRestaurantMealForm(mealPlanList);
        setOriginalRestaurantMealForm(mealPlanList);
        ...
        // console.log(restaurantMeal, "restaurantMeal");
      } else {
        const restaurantInitialValue = qoutationData?.Days?.map((day, ind) => {
          return {
            ...itineraryRestaurantInitialValue,
  ...
  }, [qoutationData, apiDataLoad]);
  console.log(restaurantMealForm, "");
  ...
  }, [qoutationData, restaurantList, restaurantSupplierList]);
  ...
  console.log(restaurantFormValue, "restaurantFormValue");
  ...
  const createFilteredFinalForm = async () => {
    // Transform meal form data to set IsMealType, ServiceID, and ServiceType
    ...
    const finalFormData = restaurantFormValue?.map((form, idx) => {
      const mealPlan = Array.isArray(restaurantMealForm[idx])
        ? restaurantMealForm[idx]
        : [];
      const totalCostingMealPlan = ["Breakfast", "Lunch", "Dinner"].map(
        (mealType) => {
          const meal = mealPlan.find((m) => m?.MealType === mealType) || {};
          const serviceCost = mathRoundHelper(
            (supplementRate?.Price?.[mealType] || 0).toFixed(2)
          );
          const markupValue = mathRoundHelper(RestaurantData?.Value || 0);
          ...
          console.log(
            serviceCost,
            `serviceCost for ${mealType} on Day ${idx + 1}`
          );
          return {
            MealType: mealType,
      ...
      const validMeals = Array.isArray(restaurantMealForm[idx])
        ? restaurantMealForm[idx].filter(
            (meal) => meal?.HotelId !== "" || meal?.RestaurantId !== ""
          )
        : [];
      const cleanedMealPlan = validMeals.map(
        ({ IsMealType, RestaurantId, HotelId, IsHotel, Amount, ...rest }) => ({
          ...rest,
      ...
      return {
        ...
        Hike: hikePercent,
        DayType: Type,
        MealPlan: filteredMeal[idx] || [],
        Sector: fromToDestinationList[idx] || [],
        TotalCostingMealPlan: totalCostingMealPlan,
      };
    });

    // Calculate total cost
    const totalCost = finalFormData.reduce((total, item) => {
    ...
    // Filter valid forms
    const filteredFinalForm = finalFormData
      ?.map((form) => {
        const validMeals = Array.isArray(form?.MealPlan)
          ? form.MealPlan.filter(
              (meal) => meal?.HotelId !== "" || meal?.RestaurantId !== ""
            )
          : [];
    ...
    // Dispatch the filtered form
    dispatch(setRestaurantFinalForm(filteredFinalForm));

    // Send data to server

    return filteredFinalForm;
  };

  // Update useEffect to handle async function
  useEffect(() => {
    console.log("createFilteredFinalForm called");
    createFilteredFinalForm();
  }, [
    restaurantFormValue,
    restaurantMealForm,
    hikePercent,
