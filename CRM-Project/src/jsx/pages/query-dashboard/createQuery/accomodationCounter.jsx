...
// const disallowedRoomIds = [7, 8];
const AccomodationCounter = ({
  value,
  roomId,
  dispatch,
  counter,
  ...
  const handleCounterInputValue = (e) => {
    ...
    // if (disallowedRoomIds.includes(roomId)) return;
  ...
  useEffect(() => {
    if (!isNaN(value) && value >= 0) {
      dispatch({ type: "SET", value: parseInt(value), counter });
    }
  }, [value, counter]);
    ...
    <>
      ...
      <div className="d-flex justify-content-between align-items-center pt-1 mt-1">
        <div
          ...
          onClick={() => {
            // if (disallowedRoomIds.includes(roomId)) return;
            dispatch({ type: "SET", value: 1, counter });
            onClick(counter, 1);
          }}
        ...
        <div
          ...
          onClick={() => {
            // if (disallowedRoomIds.includes(roomId)) return;
            dispatch({ type: "SET", value: 2, counter });
            onClick(counter, 2);
          }}
          style={{ height: "19px", fontSize: "9px" }}
        >
        ...
        <div
          ...
          onClick={() => {
            // if (disallowedRoomIds.includes(roomId)) return;
            dispatch({ type: "SET", value: 3, counter });
            onClick(counter, 3);
          }}
          // style={{ height: "19px", fontSize: "9px" }}
        >
        ...
        <div
          ...
          onClick={() => {
            // if (disallowedRoomIds.includes(roomId)) return;
            dispatch({ type: "SET", value: 4, counter });
            onClick(counter, 4);
          }}
          // style={{ height: "19px", fontSize: "9px" }}
        >
        ...
        <div
          ...
          onClick={() => {
            // if (disallowedRoomIds.includes(roomId)) return;
            dispatch({ type: "SET", value: 5, counter });
            onClick(counter, 5);
          }}
          // style={{ height: "19px", fontSize: "9px" }}
        >
...
export default AccomodationCounter;
