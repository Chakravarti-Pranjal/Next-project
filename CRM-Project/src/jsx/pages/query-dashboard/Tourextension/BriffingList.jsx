...
function BriffingList({ setActiveTab }) {
  ...
  const handleEdit = (data) => {
    navigate("/query/tour-execution/brifing-sheet", {
      state: data,
    });
  };
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("delete-briffing-sheet", {
          id: id,
        });
  ...
  const table_columns = [
    {
      name: "Name of Group",
      selector: (row) => row?.BriffingSheetDetails?.NameOfGroup,
      cell: (row) => <span>{row?.BriffingSheetDetails?.NameOfGroup} </span>,
      sortable: true,
      width: "21rem",
      wrap: true,
    },
    {
      name: "Name of Foreign Agent",
      selector: (row) => row?.BriffingSheetDetails?.NameOfForeignerAgent,
      cell: (row) => <span>{row?.BriffingSheetDetails?.NameOfForeignerAgent} </span>,
      sortable: true,
      width: "21rem",
      wrap: true,
    },
    {
      name: "File Code",
      selector: (row) => row?.BriffingSheetDetails?.FileCode,
      cell: (row) => <span>{row?.BriffingSheetDetails?.FileCode} </span>,
      sortable: true,
      width: "21rem",
      wrap: true,
    },
    {
      name: "Name of Tour Leader",
      selector: (row) => row?.BriffingSheetDetails?.TourLeader,
      cell: (row) => <span>{row?.BriffingSheetDetails?.TourLeader} </span>,
      sortable: true,
      width: "21rem",
      wrap: true,
    },
    ...
    <Tab.Container defaultActiveKey="All">
      ...
      <div className="row">
        <div className="col-12 d-flex gap-3 justify-content-end w-100">
          ...
          <button
            onClick={() => navigate("/query/tour-execution/brifing-sheet")}
            className="btn btn-primary btn-custom-size"
          >
            Add New Briefing Sheet
          </button>
          <button
            className="btn btn-primary btn-custom-size "
            name="SaveButton"
