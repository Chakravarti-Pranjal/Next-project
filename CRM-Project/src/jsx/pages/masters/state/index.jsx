...
import { stateValidationSchema } from "../master_validation.js";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../css/custom_style";
import { stateInitialValue } from "../masters_initial_value.js";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(stateInitialValue);
  ...
  const table_columns = [
    ...
    },
    {
      ...
      cell: (row) => <span>{row?.StateCode}</span>,
      sortable: true,
      width: "15rem",
    },
    {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await stateValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      //  console.log(formValue);
      const { data } = await axiosOther.post("addupdatestate", formValue);
      if (data?.Status == 1) {
        getListDataToServer();
        setIsEditing(false);
        setFormValue(stateInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deletestate", {
          id: id,
        });
