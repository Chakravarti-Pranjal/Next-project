...
import { commisionValidationSchema } from "../master_validation.js";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../css/custom_style";
import { commisionCostInitialValue } from "../masters_initial_value.js";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(commisionCostInitialValue);
...
const navigate = useNavigate();
  ...
  const getListDataToServer = async () => {
    try {
      const { data } = await axiosOther.post("commissionlist");
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        data?.CommissionName?.toLowerCase()?.includes(
          filterInput?.toLowerCase()
        ) ||
        data?.CommissionPercentage?.toLowerCase()?.includes(
          filterInput?.toLowerCase()
        ) ||
  ...
  const table_columns = [
    ...
    },
    {
      ...
      selector: (row) => row?.CommissionName,
      cell: (row) => <span>{row?.CommissionName} </span>,
      sortable: true,
    },
    {
      name: "Commission Percent(%)",
      selector: (row) => row?.CommissionPercentage,
      cell: (row) => <span>{row?.CommissionPercentage} </span>,
      sortable: true,
    },
    {
      name: "Status",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await commisionValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      const { data } = await axiosOther.post("addupdatecommission", formValue);
      if (data?.Status == 1) {
        getListDataToServer();
        setFormValue(commisionCostInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deletecommission", {
          id: id,
        });
                      ...
                      <div className="row form-row-gap">
                        <div className="col-md-6 col-lg-3">
                          ...
                          <input
                            ...
                            value={formValue?.CommissionName}
                            onChange={handleFormChange}
                          />
                          {validationErrors?.CommissionName && (
                            <div
                        ...
                        <div className="col-md-6 col-lg-3">
                          ...
                          <input
                            ...
                            placeholder="Enter a commission percent"
                            value={formValue?.CommissionPercentage}
                            onChange={handleFormChange}
                          />
