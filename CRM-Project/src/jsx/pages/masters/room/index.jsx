...
import { roomMasterValidationSchema } from "../master_validation.js";
import { roomMasterInitialValue } from "../masters_initial_value.js";
...
const RoomType = () => {
  ...
  const [formValue, setFormValue] = useState(roomMasterInitialValue);
  ...
  const handleReset = () => {
      setFormValue(roomMasterInitialValue);
        setValidationErrors({});
        setIsEditing(false);
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await roomMasterValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      //  console.log(formValue);
      const { data } = await axiosOther.post("addupdateroom", formValue);
      if (data?.Status == 1) {
        getListDataToServer();
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deleteroom", {
          id: id,
        });
