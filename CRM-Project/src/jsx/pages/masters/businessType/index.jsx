...
import { businessTypeValidationSchema } from "../master_validation.js";
import { businessTypeInitialValue } from "../masters_initial_value.js";
...
const Language = () => {
  ...
  const [businessData, setBusinessData] = useState(businessTypeInitialValue);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await businessTypeValidationSchema.validate(
        { ...businessData },
        { abortEarly: false }
      );
      ...
      const { data } = await axiosOther.post("addupdatebusinesstype", {
        ...businessData,
      });
      if (data?.Status == 1) {
        getListDataToServer();
        setBusinessData(businessTypeInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deletebusinesstype", {
          id: id,
        });
