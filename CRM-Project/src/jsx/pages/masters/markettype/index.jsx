...
import { marketValidationSchema } from "../master_validation.js";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../css/custom_style";
import { marketTypeInitialValue } from "../masters_initial_value.js";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(marketTypeInitialValue);
  ...
  const handleReset = () => {
    setFormValue(marketTypeInitialValue);
    setValidationErrors({});
    setIsEditing(false);
  };
  const handleSubmit = async (e) => {
    ...
    try {
      await marketValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      //  console.log(formValue);
      const { data } = await axiosOther.post("addupdatemarkettype", formValue);
      if (data?.Status == 1) {
        getListDataToServer();
        setFormValue(marketTypeInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deletemarkettype", {
          id: id,
        });
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Add Market Type</h4>
              <button
