...
import { CabintypeValidationSchema } from "../../master_validation.js";
import { CabintypeInitialValue } from "../../masters_initial_value.js";
...
const Cabintype = () => {
  ...
  const [CruiseNamelist, setCruiseNamelist] = useState([]);
  const [formValue, setFormValue] = useState(CabintypeInitialValue);
  ...
  const getListDataToServer = async () => {
    try {
      const { data } = await axiosOther.post("cabintypemasterlist");
      setInitialList(data?.DataList);
      setFilterValue(data?.DataList);
    } catch (error) {
      console.log("cabintypemasterlist-error", error);
    }
  };
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        ...
        data?.CabinType?.toLowerCase()?.includes(filterInput?.toLowerCase()) ||
  ...
  const table_columns = [
    ...
    },
    {
      ...
      cell: (row) => <span>{row?.CabinType}</span>,
      sortable: true,
      width: "18rem",
    },
    {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await CabintypeValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      const { data } = await axiosOther.post(
        "addupdatecabintypemaster",
        formValue
      );
      if (data?.Status === 1) {
        getListDataToServer();
        setIsEditing(false);
        setFormValue(CabintypeInitialValue);
  ...
  const getDropdownDataToServer = async () => {
    try {
      ...
      setCruiseNamelist(data.DataList);
    } catch (err) {
      console.log(err);
    }
  };
          ...
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">
                {isEditing ? "Update Cabin type " : "Add Cabin type"}
              </h4>
              <button
                      ...
                      <div className="row form-row-gap">
                        <div className="col-md-6 col-lg-3">
                          ...
                          <input
                            ...
                            className={`form-control form-control-sm ${
                              validationErrors?.CruiseName ? "is-invalid" : ""
                            }`}
                            name="CruiseName"
                        ...
                        <div className="col-md-6 col-lg-3">
                          ...
                          <input
                            type="text"
                            className={`form-control form-control-sm ${
                              validationErrors?.CabinType ? "is-invalid" : ""
                            }`}
                            name="CabinType"
                            placeholder="Enter a CabinType "
                            value={formValue?.CabinType}
                            onChange={handleFormChange}
                          />
                          {validationErrors?.CabinType && (
                            <div
                              className="invalid-feedback animated fadeInUp"
                              style={{ display: "block" }}
                            >
