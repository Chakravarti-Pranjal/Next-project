...
import { passportTypeValidationSchema } from "../master_validation.js";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../css/custom_style";
import { passportTypeInitialValue } from "../masters_initial_value.js";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(passportTypeInitialValue);
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        data?.PassportType?.toLowerCase()?.includes(
  ...
  const table_columns = [
    ...
    {
      name: "Name",
      selector: (row) => row?.PassportType,
      cell: (row) => <span>{row?.PassportType} </span>,
      sortable: true,
    },
    {
      name: "Status",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await passportTypeValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      // console.log(formValue);
      const { data } = await axiosOther.post(
        "addupdate-passport-type-master",
        formValue
      );
      if (data?.Status == 1) {
        getListDataToServer();
        setFormValue(passportTypeInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("delete-passport-type", {
          id: id,
        });
