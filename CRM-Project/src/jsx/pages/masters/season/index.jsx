...
import { seasonTypeValidationSchema } from "../master_validation.js";
import { seasonTypeInitialValue } from "../masters_initial_value.js";
...
const Language = () => {
  ...
  const [businessData, setBusinessData] = useState(seasonTypeInitialValue);
  ...
  const handleReset = (seasonTypeInitialValue) => {
    setBusinessData();
    setValidationErrors({});
    setIsEditing(false);
  };
  ...
  const getFromDate = () => {
    return businessData?.FromDate ? new Date(businessData?.FromDate) : null;
  };
  const getToDate = () => {
    return businessData?.ToDate ? new Date(businessData?.ToDate) : null;
  };
  const handleToCalender = (date) => {
    //  console.log(date); // Log the selected Date object
    setBusinessData({
      ...businessData,
      ToDate: date, // Store the selected date as a Date object
    });
  };
  const handleCalender = (date) => {
    ...
    setBusinessData({
      ...businessData,
      FromDate: date, // Store the selected date as a Date object
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await seasonTypeValidationSchema.validate(
        { ...businessData },
        { abortEarly: false }
      );
      ...
      const { data } = await axiosOther.post("addupdateseason", {
        ...businessData,
      });
      console.log(businessData, "data");
      if (data?.Status == 1) {
        getListDataToServer();
        setBusinessData(seasonTypeInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deleteseason", {
          id: id,
        });
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        ...
        data?.SeasonName?.toLowerCase()?.includes(filterInput?.toLowerCase()) ||
        data?.FromDate?.toLowerCase()?.includes(filterInput?.toLowerCase()) ||
        data?.ToDate?.toLowerCase()?.includes(filterInput?.toLowerCase()) ||
  ...
  const table_columns = [
...

    {
      ...
      cell: (row) => <span>{formatDate(row?.FromDate)} </span>,
      sortable: true,
    },

    {
      ...
      cell: (row) => <span>{formatDate(row?.ToDate)} </span>,
      sortable: true,
    },
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Add Session Type</h4>
              <button
                          ...
                          <div className="d-flex gap-3">
                            <div className="form-check">
                              <input
                                ...
                                value="1"
                                id="default_yes"
                                checked={businessData?.Default == 1}
                                onChange={handleStateData}
                              />
                              <label
                            ...
                            <div className="form-check">
                              <input
                                ...
                                value="0"
                                id="default_no"
                                checked={businessData?.Default == 0}
                                onChange={handleStateData}
                              />
                              <label
                                className="form-check-label"
