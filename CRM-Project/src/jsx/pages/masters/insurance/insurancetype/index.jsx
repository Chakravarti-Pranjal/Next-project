...
//import { hotelTypeInitialValue } from "../masters_initial_value";
//import { hotelTypeValidationSchema } from "../master_validation";
import { insuranceTypeValidationSchema } from "../../master_validation";
//import DataTable from "react-data-table-component";
import DataTable from "react-data-table-component";
//import { table_custom_style } from "../../../../css/custom_style";
import { table_custom_style } from "../../../../../css/custom_style.js";
import { insuranceTypeInitialValue } from "../../masters_initial_value";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(insuranceTypeInitialValue);
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        data?.InsuranceType?.toLowerCase()?.includes(
  ...
  const table_columns = [
    ...
    },
    {
      ...
      cell: (row) => <span>{row?.InsuranceType}</span>,
      sortable: true,
    },
  ...
  const handleReset = () => {
    setFormValue(insuranceTypeInitialValue);
    setValidationErrors({});
    setIsEditing(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await insuranceTypeValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      const { data } = await axiosOther.post(
        "addupdateinsurancetypemaster",
        formValue
      );
      if (data?.Status == 1) {
        getListDataToServer();
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deleteinsurancetype", {
          id: id,
        });
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Insurance Type</h4>
              <button
