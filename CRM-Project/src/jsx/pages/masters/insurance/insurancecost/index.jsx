...
import { insuranceCostValidationSchema } from "../../master_validation";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../../css/custom_style";
import { insuranceCostInitialValue } from "../../masters_initial_value";
...
const InsuranceCost = () => {
  ...
  const [formValue, setFormValue] = useState(insuranceCostInitialValue);
  ...
  const getListDataToServer = async () => {
    try {
      const { data } = await axiosOther.post("insurancecostmasterlist");
  ...
  const postDataToServer = async () => {
    try {
      ...
      setInsuranceTypeList(data?.DataList);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        data?.InsuranceName?.toLowerCase()?.includes(
          filterInput?.toLowerCase()
        ) ||
        data?.InsuranceTypeName?.toLowerCase()?.includes(
          filterInput?.toLowerCase()
        ) ||
  ...
  const table_columns = [
...

    {
      ...
      cell: (row) => <span>{row?.InsuranceTypeName}</span>,
      sortable: true,
    },
...

    {
      ...
      selector: (row) => (
        <NavLink
          to={`/master/insurancecost/rate/${row?.id}`}
          state={{ Name: row?.TourEscortService }}
        >
  ...
  const handleReset = () => {
    setFormValue(insuranceCostInitialValue);
    setValidationErrors({});
    setIsEditing(false);
  };
  const handleSubmit = async (e) => {
    ...
    try {
      await insuranceCostValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      const { data } = await axiosOther.post(
        "addupdateinsurancecostmaster",
        formValue
      );
      if (data?.Status == 1) {
        getListDataToServer();
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deleteinsurancecost", {
          id: id,
        });
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Insurance Cost</h4>
              <button
                      ...
                      <div className="row form-row-gap">
                        ...
                        <div className="col-md-6 col-lg-3">
                          ...
                          <select
                            id=""
                            name="InsuranceType"
                            value={formValue?.InsuranceType}
                            onChange={handleInputChange}
                            className="form-control form-control-sm"
                          >
                            <option value="">Select</option>
                            {insuranceTypeList?.map((value, index) => {
                              return (
