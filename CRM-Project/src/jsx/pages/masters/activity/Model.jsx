...
const Model = ({
  ...
  console.log(ImageContainer, "ImageContainer111");
  ...
  return (
    <Modal show={open} onHide={() => setopen(false)} centered size="xl">
      <Modal.Header closeButton>
        <Modal.Title>Activity Images List ({row?.DestinationName}) Activity Images Dimension ( 318 * 543 )</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <div className="row">
          <div className="col-12 mb-3">
            <ImageContainer validateSize={true} targetWidth={371} targetHeight={413} />
        ...
        <div className="mt-3">
          <Table striped bordered responsive>
            ...
            {serviceimagelist && serviceimagelist.length > 0 ? (
              serviceimagelist?.map((image, index, key) => {
                return (
                  <tbody>
                    <tr value={image?.id} key={index}>
                      <td> {index + 1}</td>
                      <td>
                      ...
                      <td className="text-center">
                        {" "}
                        <i
                          className="fa-solid fa-trash-can cursor-pointer text-danger action-icon sweet-confirm ms-auto me-auto "
                          onClick={() =>
                            image?.id && handleimageDelete(image?.id)
                          }
                        ></i>
                      </td>
                    </tr>
