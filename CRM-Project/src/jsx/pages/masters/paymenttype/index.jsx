...
import { paymentTypeValidationSchema } from "../master_validation.js";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../css/custom_style";
import { paymentTypeInitialValue } from "../masters_initial_value.js";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(paymentTypeInitialValue);
...
const handleReset = () => {
                  ...
                  setFormValue(paymentTypeInitialValue);
                  setValidationErrors({});
                  setIsEditing(false);
                };
  useEffect(() => {
  ...
  useEffect(() => {
    const filteredList = initialList?.filter(
      (data) =>
        data?.PaymentTypeName?.toLowerCase()?.includes(
  ...
  const table_columns = [
    ...
    {
      name: "Name",
      selector: (row) => row?.PaymentTypeName,
      cell: (row) => <span>{row?.PaymentTypeName} </span>,
      sortable: true,
    },
    {
      name: "Added By",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await paymentTypeValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      //  console.log(formValue);
      const { data } = await axiosOther.post("addupdatepaymenttype", formValue);
      if (data?.Status == 1) {
        getListDataToServer();
        setFormValue(paymentTypeInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deletepaymenttype", {
          id: id,
        });
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Add Payment Type</h4>
              <button
                      ...
                      <div className="row form-row-gap">
                        <div className="col-md-6 col-lg-3">
                          ...
                          <input
                            ...
                            value={formValue?.PaymentTypeName}
                            onChange={handleFormChange}
                          />
                          {validationErrors?.PaymentTypeName && (
                            <div
