...
import { expenceHeadValidationSchema } from "../master_validation";
import DataTable from "react-data-table-component";
import { table_custom_style } from "../../../../css/custom_style";
import { expenceHeadInitialValue } from "../masters_initial_value";
...
const InsuranceType = () => {
  ...
  const [formValue, setFormValue] = useState(expenceHeadInitialValue);
  ...
  const handleReset = () => {
    ...
    setFormValue(expenceHeadInitialValue);
    setValidationErrors({});
    setIsEditing(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await expenceHeadValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      // console.log(formValue);
      const { data } = await axiosOther.post("addupdateexpensehead", formValue);
      if (data?.Status == 1) {
        getListDataToServer();
        setFormValue(expenceHeadInitialValue);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deleteexpensehead", {
          id: id,
        });
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Expense Head</h4>
              <button
