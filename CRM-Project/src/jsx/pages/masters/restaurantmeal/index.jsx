...
import { restaurantMealPlanInitialValue } from "../masters_initial_value";
import { restaurantMealPlanValidationSchema } from "../master_validation";
...
const RestaurantMeal = () => {
  ...
  const [formValue, setFormValue] = useState(restaurantMealPlanInitialValue);
  ...
  const getListDataToServer = async () => {
    try {
      ...
      console.log("restaurantmeallist-error", error);
    }
  };
  useEffect(() => {
    getListDataToServer();
  ...
  const handleReset = () => {
    setFormValue(restaurantMealPlanInitialValue);
    setValidationErrors({});
    setIsEditing(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await restaurantMealPlanValidationSchema.validate(formValue, {
        abortEarly: false,
      });
      setValidationErrors({});
      //  console.log(formValue);
      const { data } = await axiosOther.post(
        "addupdaterestaurantmeal",
        formValue
      );
      if (data?.Status == 1) {
        getListDataToServer();
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        const { data } = await axiosOther.post("deletemeal", { id });
        if (data?.Status === 1 || data?.status === 0 || data?.result) {
          notifySuccess(data?.Message || data?.message || data?.result);
          getListDataToServer();
        } else {
          notifyError(data?.Message || data?.message || "Deletion failed");
        }
      } catch (err) {
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Resturant Meal</h4>
              <button
